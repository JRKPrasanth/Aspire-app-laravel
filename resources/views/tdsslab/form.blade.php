...
@section('content')
<h3 class="text-danger">TDS Slab(%)</h3>
...
<div class="card-body card-block">
    <form id="tds_form" action="" data-parsley-validate>
        <?php $data = \Session::get('data');
        if (isset($data[$pageMethod]['save'])) { ?>

            <input type="hidden" value="" name="savestatus" id="savestatus" />

            <input type="hidden" name="edit_id" value="{{$row->tds_slab_id}}" id="edit_id" /> {{ csrf_field()}}
                ...
                <div class="col-md-12">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="row mb-3">
                                <label for="inputIsValid" class="col-form-label col-md-5">
                                    <span style="color:red;">*</span> TDS Percentage</label>
                                <div class="col-md-7">
                                    <input type="text" name="tds_percentage" id="tds_percentage"
                                        value="{{$row->tds_percentage}}" class="form-control tds_percentage" required
                                        tabindex="1">
                        ...
                        <div class="col-md-4">
                            <div class="row mb-3">
                                <label for="inputIsValid" class="col-form-label col-md-5">
                                    <span style="color:red;">*</span> TDS Types</label>
                                <div class="col-md-7">
                                    <input type="text" name="tds_type" id="tds_type" value="{{$row->tds_type}}"
                                        class="form-control tds_type" required tabindex="2">
                                </div>
                            </div>
                            <div class=" row mb-3" style="display:none;">
...
<script>
    ...
    $(document).ready(function () {
      var table = $('#AccountsTbl').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('getTdsslabData') }}",
        columns: [

          { data: 'tds_percentage', name: 'tds_percentage' },
          { data: 'tds_type', name: 'tds_type' },
...

          {
            ...
            render: function (data, type, row) {
              ...
              if (window.toolbarButtons?.some(btn => btn.attr.id === 'edit-btn')) {
                buttons += `
          <button class="btn btn-sm btn-primary edit-btn" data-id="${row.tds_slab_id}" data-percentage="${row.tds_percentage}" data-type="${row.tds_type}" data-active="${row.active}">
    ...
    $.ajax({
        cache: false,
        url: "{{ URL::to('tdsslabcheckname/') }}",
        type: 'GET',
        dataType: 'json',
        async: false,
        data: {tds_percentage: tds_percentage, edit_id: edit_id},
        success: function (response){
            if (response == 1)
            {
                $('.dup_name').html('TDS Percentage:' + tds_percentage + ' Already Exists');
                $('.dup_name').show();
                $(".tds_percentage").val('');
                dup_chk = false;
...
// edit function
    ...
    $('input[name="tds_percentage"]').val(percentage);
    $('input[name="tds_type"]').val(type);
    $('select[name="active"]').val(active).trigger('change');
		 
	}else{
		
       showCustomAlert("You Can't be Edit this TDS, Already used","warning");
	...
	$(document).on('keypress change', '.tds_percentage', function(ev){
  ...
  $('.tds_percentage').bind("cut copy paste", function(e) {
        e.preventDefault();
            });
...
// delete function
	...
	$('#globalConfirmDeleteBtn').on('click', function () {
		if (deleteId) {
				$.ajax({
					url: "{{ url('tdsslabdelete') }}/" + deleteId,
					type: "GET",
