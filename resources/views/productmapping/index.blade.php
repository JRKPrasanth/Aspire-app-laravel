  ...
  <div class="card shadow-lg rounded-4 border-0 p-3 mb-4">
    <div class="card-body">
      ...
      <div class="table-responsive">
        <table id="promappingTbl" class="table table-bordered table-striped w-100">
          <thead>
...
@push('scripts')
  <script>
    $(document).ready(function () {
      ...
      var table = $('#promappingTbl').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('getproductmappingdata') }}",
        columns: [
          { data: 'employee_number', name: 'hr_employee_t.employee_number' },
          { data: 'first_name', name: 'hr_employee_t.first_name' },
          { data: 'description', name: 'productmapping_hdr_tbl.description' },
          {
            data: 'productmapping_id',
            name: 'actions',
            orderable: false,
            searchable: false,
      ...
      $('#promappingTbl thead').on('keyup change', ".column-search", function () {
    ...
    $(".create").click(function () {
      var url = "{{ URL::to('productmappingedit/0')}}";
      window.location.replace(url);
    });
    //edit function
    $(document).on('click', '.edit-btn', function () {
      const id = $(this).data('id');
      const url = "{{ url('productmappingedit') }}/" + id;
    ...
    $(document).on('click', '.view-btn', function () {
      const id = $(this).data('id');
      const url = "{{ url('productmappingview') }}/" + id;
      window.location.href = url;
    });
    ...
    $('#globalConfirmDeleteBtn').on('click', function () {
      if (deleteId) {
        $.ajax({
          url: "{{ url('productmappingdelete') }}/" + deleteId,
          type: "GET",
          success: function (response) {
            $('#globalDeleteModal').modal('hide');
            showCustomAlert('Deleted successfully!', 'success');
            $('#promappingTbl').DataTable().ajax.reload();
