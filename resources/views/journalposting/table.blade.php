...
<script>
...

    {
            ...
            render: function (data, type, row) {
              let buttons = '';
              if (window.toolbarButtons?.some(btn => btn.attr.id === 'post')) {
                buttons += `
        <button class="btn btn-sm btn-primary post-btn" data-id="${row.journal_entry_id}">
         Posting
        </button>`;
              }
...
$(document).on('click', '.post-btn', function () {
      ...
      const url = "{{ url('journalpostingview') }}/" + id;
