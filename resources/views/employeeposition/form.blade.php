...
@section('content')
  <h3 class="text-danger">Employee Grade</h3>
  ...
  if (isset($data[$pageMethod]['save'])) { ?>
        ...
        <div class="row g-4">
          <!-- Position -->
          <div class="col-md-4">
            <label for="position" class="form-label"><span class="text-danger">*</span> Grade</label>
            <input type="text" id="position" name="position" class="form-control position" required>
          ...
          <div class="col-md-4">
            ...
            <input type="text" id="position_description" name="position_description"
              class="form-control position_description">
          </div>
...
@push('scripts')
  ...
  <script>
          ...
          { data: 'active', name: 'active' },
          {
            ...
            render: function (data, type, row) {
              ...
              <button type="button" class="btn btn-sm btn-primary edit-btn"
                ...
                data-name="${row.position_description}"
    ...
    function duplicate_validate() {
      ...
      $.ajax({
        cache: false,
        url: 'employeeposition/checkname',
        type: 'GET',
        dataType: 'json',
        async: false, // blocking check
        data: { position: position, edit_id: edit_id },
        success: function (response) {
          if (response == 1) {
            $('.dup_name')
              .html('Position: ' + position + ' already exists')
              .removeClass('d-none')
              .addClass('d-block');
    ...
    $(document).on('click', '.saveform', function () {
      ...
      if (form.parsley().isValid() && dup_chk == true) {
        ...
        $.ajax({
          url: "{{ URL::to('employeeposition/save') }}",
          type: "POST",
          data: form.serialize(),
    ...
    $(document).on('click', '.edit-btn', function () {
      ...
      $('input[name="position"]').val(code);
      $('input[name="position_description"]').val(name);
      $('#edit_id').val(id);
    ...
    $('#globalConfirmDeleteBtn').on('click', function () {
      if (deleteId) {
        $.ajax({
          url: "{{ url('employeeposition/delete') }}/" + deleteId,
          type: "GET",
          success: function (response) {
    ...
    $(document).on('keypress', '#position', function (ev) {
