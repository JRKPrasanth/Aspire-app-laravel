...
@section('content')
  <h3 class="text-danger">Payment For Employee Bonus</h3>
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                <select type="text" name="payment_source" id="payment_source" class="form-control  payment_source"
                    readonly>
                    <option <?php if ($row->payment_source == "BONUSPAYMENT")
                        echo "selected"; ?> value="BONUSPAYMENT">
                        BONUSPAYMENT</option>
                </select>
            </div>
            <div class="col-md-2">
	 ...
	 /* Purpose For Save Function*/      
		...
		$(document).on('click', '.saveform', function() {
		  ...
		  var payment_amount=$('.payment_amount').val();
			...
			var url = "{{ url('bonuspaysave') }}";
			var red_url = "{{url('paymentforbonus')}}"
