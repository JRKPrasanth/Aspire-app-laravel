...
@section('content')
<h3 class="text-danger">HQ wise Person Productivity</h3>
    <style>
 .sticky-col {
...
<div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
      <form action="{{ url('asmpersonproduction') }}" method="get" id="searchForm">
        <div class="row g-3">
...
<div class="card shadow-lg rounded-4 border-0 p-4">
  ...
  <?php endif; ?>
                    <th colspan="7" class="align text-white bg-danger text-center">Personwise Productivity UPTO - {{ $mon_yr }}</th>
    ...
    <div class="table-responsive" style="overflow-x: auto;">
        ...
        <thead>
            ...
            <?php if (request('area') != ''): ?>
              <th class="align text-white bg-danger text-center">{{ request('area') }}</th>
              <?php endif; ?>
                <th colspan="64" class="align text-white bg-danger text-center">Monthly Productivity upto  {{ $mon_yr}}</th>
                 </tr>
                <tr>
        <th colspan="3" class="align text-white bg-secondary text-center" ></th>
        <?php $Zones = []; ?>
        <?php foreach ($zone_wise as $value) {
            ...
            if (!in_array($zone_type, $Zones)) {
                $Zones[] = $zone_type; ?>
                <th colspan="3" class="text-white bg-secondary"><?php echo $zone_type; ?></th>
        <?php }
        } ?>
     </tr>
         <tr>
        <th  class="sticky-col text-white bg-success">Hq Name</th>
        <th  class="sticky-coll text-white bg-success">Currrent Reporting MGR</th>
        <th  class="sticky-colll text-white bg-success">Field Force Name</th>
        <?php foreach ($Zones as $zone) { ?>
            <th class="bg-warning">Sale</th>
            <th class="bg-warning">Target</th>
            <th class="bg-warning">Achivement %</th>
        <?php } ?>
    </tr>
...
?>
    <tr>
        <td  class='sticky-col'><?php echo $state; ?></td>
        ...
        foreach ($Zones as $zone): 
            $monthData = array_filter($zone_wise, function ($item) use ($zone, $state, $mgr, $person) {
                return $item->month_y == $zone && $item->hq_name == $state && $item->current_reporting_manager == $mgr && $item->field_force_name == $person;
            });
            ...
            ?>
                ...
                <td ><?php echo ($item->target != 0) ? number_format($item->sale / $item->target * 100, 0) . '%' : '0'; ?></td>
            <?php else: ?>
                <td>0</td>
                <td>0</td>
                <td >0%</td>
