...
@section('content')
    <h3 class="text-danger">Approve Bonus</h3>
    @include('layouts.breadcrumb')

    <button type='button' class='btn btn-success mt-4 approved_selected'><i class="bi bi-check2-circle"></i> Approve
        Selected</button>
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.approved_selected', function () {
                ...
                if (selectedIds.length > 0) {
                    var url = "{{ URL('approvedbonus') }}/?row_id=" + selectedIds.join(',');
                    $.get(url, function (data) {
                        if (data == 1) {
                            showCustomAlert('Bonus Approved Successfully', 'success');
                            table.ajax.reload();
                        }
                    });
                } else {
            ...
            $(".clear").click(function () {
                $('#BonusTbl').DataTable().search('').draw();
            });
