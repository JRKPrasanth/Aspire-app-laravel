...
$("#grid1").jqGrid({
url:"{{URL::to('institutionData')}}",
mtype:'GET',
datatype:'json',

colModel: [
{ name: "institute_id", label: "S.NO",hidden:true },
{ name: "name", label: " Name" },
{ name: "mail1", label: "Mail1"},
{ name: "key_contact1", label: "Key Contact1",editable:true, editrules:{date:true}},
{ name: "contact_name1", label: "Contact Name1",editable:true, editrules:{date:true}},
...
$(".edit").click(function(){
	...
	var edit_id = $("#grid1").jqGrid ('getCell', index, 'institution_id');
	
       if( edit_id){
       	alert("sdf");
		window.location.replace('institutionedit/'+edit_id);
	}
	else
	{
...
showcolumn('grid1');
	...
	if(id)
	{
		window.location.replace('institutionview/'+id );
	}
	else
	{
			 notyMsgs("info","Please Select a Row");
...
showcolumn('grid1');
	...
	else
	{
       ...
       $(".delete").click(function(){
		...
		if( pohdrid != false ){
		window.location.replace('institudelete/' +pohdrid);
		}
		else{
