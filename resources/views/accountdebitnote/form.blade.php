...
.bulk_product_id {width: 100px !important;}
.bulk_uom_code_id {width: 100px !important;}
.bulk_reject_qty {width: 100px !important;}
...
.bulk_comments {width: 100px !important;}
}
...
<h2 class="heads">Debit Note <span class="ui_close_btn"><a class="collapse-close pull-right btn-danger" onclick="location.href = '{{url('accountdebitnote')}}'"></a></span></h2>


<form method="post"  id="debit_form" data-parsley-validate>
{{ csrf_field() }}
...
<div class="card-body card-block">
    ...
    <div class="row">
	<div class="col-md-4">
		<input class="form-control " name="invoice_number" size="16" type="hidden" value="{{ $row[0]->po_invoice_id }}" >
		 	<div class="form-group row">
			<label for="inputIsValid" class="form-control-label col-md-4">Debit Number</label>
			<div class="col-md-6">
				<input type="text" id="debit_number" name="debit_number"  class="form-control" value="{{$row[0]->debit_number}}"  >
			</div>
			<div class="col-md-2">
			</div>
		</div>
                 <div class="form-group row">
                  <label for="inputIsValid" class="form-control-label col-md-4"><span style="color:red;">*</span> Debit Date</label>
			<div class="col-md-6">
			<div class="input-group date form_date col-md-12" data-date="" data-date-format="dd MM yyyy" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd">
			<input class="form-control debit_date" id="debit_date" name="debit_date" size="16" type="text" value="{{ $row[0]->debit_date }}" required >
			<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
			</div>
			<input type="hidden" id="debit_date" value="{{ $row[0]->debit_date }}" />
			</div>
			<div class="col-md-1 showline">
                ...
                <div class="form-group row" >
        <label for="inputIsValid" class="form-control-label col-md-4">Debit Status</label>
        <div class="col-md-6">
            <select type="text" name="debit_status" id="debit_status" class="form-control debit_status" readonly>
                <option value="">--Please Select--</option>
                <option <?php if($row[0]->debit_status =="DRAFT") { echo "selected"; } else { echo ""; } ?> value="DRAFT">DRAFT</option>
                <option <?php if($row[0]->debit_status =="INITIATED") { echo "selected"; } else { echo ""; } ?> value="INITIATED">INITIATED</option>
            </select>
        </div>
    </div>
	...
	<div class="col-md-4">
		<div class="form-group row">
			...
			<div class="col-md-6">
			<input class="form-control debitnote_hdr_id" id="debitnote_hdr_id" name="debitnote_hdr_id" size="16" type="hidden" value="{{ $row[0]->debitnote_hdr_id }}" readonly>
				<input class="form-control " id="po_hdr_id" name="po_number" size="16" type="hidden" value="{{ $row[0]->po_hdr_id }}" >
				<input type="text" id="po_number"   class="form-control po_number" value="{{$row[0]->po_number   }}" required>
			</div>
			<div class="col-md-2">
			</div>
		</div>	 	
       <div class="form-group row">
			...
			<div class="col-md-6">
				...
				<input type="text" id="bill_number" name="bill_number"  class="form-control bill_number" value="{{ $row[0]->bill_number   }}" required>
			</div>
			<div class="col-md-2">
			</div>
		</div>
    ...
    <div class="form-group row">
                        ...
                        <div class="col-md-6 ssite_div">
                            <select name='suppliersite_id' rows='5'   class='suppliersite_id form-control' readonly>
                                {!! $suppliersite_id !!}
                            </select>
                        </div>
                    <div class="col-md-2 showinline">
...
<tr class="clone clonedInput">
    <td>
        <input type="hidden" name="bulk_debitnote_line_id[]" class="form-control input-sm bulk_debitnote_line_id" value="">
    </td>
    <td>
        <input type="hidden" name="bulk_debitnote_hdr_id[]" class="form-control input-sm bulk_debitnote_hdr_id" value="">
    </td>
    <td>
	...
	<div class="col-lg-12 col-md-12">
		<div class="form-group text-center">
			...
			<button type="button" class="btn save saveform" value="SAVE">Save</button>
			  <a href="{{ url('accountdebitnote') }}" class='btn cancel'>Cancel</a>
		</div>
	</div>
</div>
...
$('#debit_date,.bulk_comments').attr('readonly', false);

$(document).ready(function(){
    $('.bulk_product_id,.bulk_uom_code_id,.bulk_tax_group_id,.supplier_id,.invoice_date,.debit_status,.suppliersite_id').css('pointer-events','none');
 $('#savestatus').val('');
    $(document).on('click','.saveform',function(){
            var btnval		= $(this).val();

            if(btnval == 'APPLYCHANGES')
                $("#debit_status").val('DRAFT');
            else if(btnval == 'DRAFT')
                $("#debit_status").val('DRAFT');
            else
                $("#debit_status").val('INITIATED');


            var url		= "{{ url('accountdebitnotesave') }}";
            var red_url		="{{ url('accountdebitnote') }}";
            var create_url	="{{ url('accountdebitnotecreate') }}/0";
            validationrule('debit_form');
            var formdata	= $('#debit_form').serialize();
            var form = $('#debit_form');
            ...
            if(btnval != 'APPLYCHANGES')
            {
                ...
                if (form.parsley().isValid())
                {
                    $.post(url,formdata,function(data)
                    {
                        ...
                        var id          = data.id;
                        var edit_url	= "{{ url('accountdebitnotecreate') }}/"+id;
            ...
            $.post(url,formdata,function(data)
            {
                    ...
                    var id     = data.id;
                    var edit_url	="{{ url('accountdebitnoteedit') }}/"+id;
                            notyMsg(status,msg);
                            setTimeout(function(){
...
$(document).on('click','.add_row',function()
{
	...
	cloned.appendTo('.po_lines_body');
        ...
        changeClassName('bulk_debitnote_line_id');
...
$(document).on('click','.remove',function()
{
	...
	if(rowCount > 1)
	{
		$($(this).closest("tr")).remove();
                removeClass('bulk_debitnote_line_id');
...
$('.debit_date').datepicker({format: 'yyyy-mm-dd', autoClose: true})
