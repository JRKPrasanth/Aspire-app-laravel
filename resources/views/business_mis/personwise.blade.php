...
@section('content')
    ...
    <div class="container mt-4">
        ...
        <div class="row g-3 mb-2">
            ...
            <div class="col-12 col-md-3">
                <a href="primarybusinessmisstate" class="btn btn-outline-success w-100">State Wise</a>
            </div>
            <div class="col-12 col-md-3">
                <a href="primarybusinessmisperson" class="btn btn-success w-100">Product wise Productivity</a>
            </div>
        </div>
    </div>

    <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
        <form action="{{ url('primarybusinessmisperson') }}" method="get" id="searchForm">
            <div class="row g-3">
            ...
            <table id="Table1" class="table table-bordered table-striped table-hover w-100">
                <thead>
                    <tr>
                        ...
                        <th colspan="24" class="align text-white bg-danger text-left"> STOCKIST SALE UPTO - {{ $mon_yr }}
                        </th>
                    ...
                    <tr>
                        ...
                        <th class="align sticky-col text-white bg-success text-center">Division</th>
                        <th class="align sticky-coll text-white bg-success text-center">Per person Per Month Avg. Sale Unit
                        </th>

                        <?php $displayedMonths = []; ?>
                        <?php foreach ($person_wise as $value) {
        $monthYear = $value->state;
    ...
    foreach ($person_wise as $value) {
        $totalStock += $value->sample;

        // Store sales data for each month
        $monthlySales[$value->division][$value->product_name][$value->state] = $value->sample;
    }

    // Loop through unique F_Year, HQ Name, Zone, Region, State combinations
    foreach ($monthlySales as $division => $product_names) {
        foreach ($product_names as $product_name => $monthData) {

            echo '<tr>';
            echo '<td class="sticky-col">' . $division . '</td>';
            echo '<td  class="sticky-coll">' . $product_name . '</td>';
    ...
    $grandTotal = 0; // Initialize grand total for each column

    foreach ($displayedMonths as $month) {
        $columnTotal = 0; // Initialize column total for each month

        foreach ($monthlySales as $division => $product_names) {
            foreach ($product_names as $product_name => $monthData) {
                $columnTotal += isset($monthData["$month"]) ? $monthData[$month] : 0;
            }
        }

        echo '<td>' . $columnTotal . '</td>';
        $grandTotal += $columnTotal; // Update grand total
    }
