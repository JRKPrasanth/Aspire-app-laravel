...
@section('content')
  ...
  <h3 class="text-danger">Secondary Data Upload</h3>


  <div class="card shadow-lg rounded-4 border-0">
    <div class="card-body">
      <form method="post" action="" id="secondarydataupload" enctype="multipart/form-data">
        {{ @csrf_field() }}
        ...
        <div class="row g-3 mb-4 align-items-end">
          <!-- Download Template -->
          <div class="col-md-3">
            <a href="{{ asset('Uploads/SECONDARY_DB_TEMPLATE.csv') }}" class="btn btn-outline-primary w-100" download>
  ...
  <div class="card shadow-lg rounded-4 border-0">
    <div class="container mt-4">
      <table id="SecTbl" class="table table-bordered table-striped w-100">
        <thead>
  ...
  <script>
    ...
    $(document).ready(function () {
      var table = $('#SecTbl').DataTable({
        ...
        ajax: "{{ route('getsecondarydatauploadData') }}",
        columns: [
          ...
          { data: 'till_month', name: 'till_month' },
          { data: 'zone', name: 'zone' },
    ...
    $('#myModal1').on('shown.bs.modal', function () {
      ...
      $(document).on('click', '.index', function () {
        ...
        if (form.parsley().isValid() && dup_chk == true) {
          ...
          $.ajax({
            url: "{{ URL::to('secondarydatauploaddata') }}",
            type: "POST",
            data: formData,
    ...
    $(document).on('click', '.edit-btn', function () {
      ...
      const url = "{{ url('secondarydatauploadedit') }}/" + id;
