...
@section('content')
<h3 class="text-danger">Payment For Employee Expense</h3>
@include('layouts.breadcrumb')
    ...
    <div class="col-md-4">
        ...
        <div class="row mb-3">
            <label for="inputIsValid" class="col-form-label col-md-6">Payment Total</label>
            <div class="col-md-6">
                <input type="text" id="payment_amount_total" name="payment_amount_total"
                    class="form-control payment_amount_total chckclick" value="" readonly>
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                <select type="text" name="payment_source" id="payment_source" class="form-control  payment_source"
                    readonly>
                    <option <?php if ($row->payment_source == "EMPLOYEEEXPENSE")
                        echo "selected"; ?>
                        value="EMPLOYEEEXPENSE">EMPLOYEE EXPENSE</option>
                </select>
            </div>
            <div class="col-md-2">
            </div>
                ...
                <tbody class="clone_lines_body">
                    ...
                    <tr>
                        <td>
							...
							<input class="form-control bulk_reference_id" id="bulk_reference_id" name="bulk_reference_id[]" size="16" type="hidden" value="{{ $value->expense_line_id }}" readonly>

                            <select name="bulk_employee_id[]" id="bulk_employee_id" class="select2 bulk_employee_id"  >{!! $value->employee_id !!}</select>
                        </td>
                          <td>
                            <input type="text" name="bulk_expense_amount[]" class="form-control  bulk_expense_amount " value="{{$value->expense_line_amount}}" readonly >
                        </td>
                        
                             <td>
                              <input type="text" name="bulk_tds_amount[]" class="form-control  bulk_tds_amount " value="{{$value->tds_amount}}" readonly >
                             </td>
                        
                        <td>
                            <input type="text" name="bulk_payment_amount[]" class="form-control  bulk_payment_amount " value="{{$value->emp_exp_total}}" required >
                        </td>
                        <td>
                        ...
                        <td>
                            <input type="text" name="bulk_sender_information[]" class="form-control bulk_sender_information" value="" required >
                        </td>
                        <td>
    ...
    if(payment_amt > invoice_amt){
      ...
      $(".payment_amount").val(num1);
        $(".paid_amount").val(paid_amt);
          ...
          /*Validation*/
  $(document).on('keypress','.bulk_payment_amount', function(ev){
...
/*End*/
	$(document).on('keyup change','.bulk_payment_amount',function(){
    
     var index = $(this).closest("tr").index();
    var payment_amt = parseInt($('.bulk_payment_amount'+index).val());
    var expense_amt = parseInt($('.bulk_expense_amount'+index).val());
    
    if(payment_amt > expense_amt){
        $('.bulk_payment_amount'+index).val('0');
    	showCustomAlert("Payment Amount Not More Then Expense Amount",'info');    
    }
      var sum=0;
    var sumtax=0;
    var sumtotal=0;
    ...
    sum += isNaN(parseFloat($(this).val()))?0:(parseFloat($(this).val()));
     ...
     var sumtotalamt=isNaN((sumtotal))?0:(sumtotal);
     
    $('.payment_amount_total').val(sumtotalamt.toFixed(2));

});
$( ".bulk_payment_amount" ).trigger("change");
    ...
    $(document).on('click', '.saveform', function() {
      ...
      var payment_amount=$('.payment_amount').val();
        ...
        $(".payment_status").val('UNPAID');
			...
			var url = "{{ url('paymentempExpensesave') }}";
			var red_url = "{{url('paymentforempexpense')}}"
