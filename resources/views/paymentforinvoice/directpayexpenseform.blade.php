...
@section('content')
<h3 class="text-danger">Direct Expense Payment</h3>
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                <select type="text" name="payment_source" id="payment_source" class="form-control  payment_source"
                    readonly>
                    <option <?php if ($row->payment_source == "SALARYPAYMENT")
                        echo "selected"; ?> value="SALARYPAYMENT">
                        SALARYPAYMENT</option>
                    <option <?php if ($row->payment_source == "DIRECTEXPENSE")
                        echo "selected"; ?> value="DIRECTEXPENSE">
                        DIRECTEXPENSE</option>
                </select>
            </div>
            <div class="col-md-2">
            </div>
                ...
                <tbody class="clone_lines_body">
                    ...
                    <tr>
                        <td>
                            ...
                            <select name="bulk_employee_id[]" id="bulk_employee_id" class="select2 bulk_employee_id"  >{!! $value->employee_id !!}</select>
                        </td>
                        <td>
                            <input type="text" name="bulk_payment_amount[]" class="form-control  bulk_payment_amount " value="{{$value->net_salary}}"  >
                        </td>
                        <td>
                            <input type="text" name="bulk_supplier_bank_id[]" class="form-control  bulk_supplier_bank_id " value="{{$value->bank_name}}" readonly >
                        </td> 
                         <td>
                            <input type="text" name="bulk_supplier_account_name[]" class="form-control  bulk_supplier_account_name " value="{{$value->account_holder_name}}" readonly >
                        </td> 
                         <td>
                            <input type="text" name="bulk_supplier_account_no[]" class="form-control  bulk_supplier_account_no " value="{{$value->account_number}}" readonly >
                        </td> 
                         <td>
                            <input type="text" name="bulk_supplier_ifsc_code[]" class="form-control  bulk_supplier_ifsc_code " value="{{$value->ifsc_code}}" readonly >
                        </td> 
                  ...
                  <?php }  if(count($linedata) < 1 ) { ?>
                    <tr class="rcopy clone">
                        <td>
                            <input type="hidden" name="bulk_payment_id[]" class="form-control  bulk_payment_id" value="">
                            <select name="bulk_employee_id[]" id="bulk_employee_id" class="select2 bulk_employee_id"  >{!! $employee_id !!}</select>
                        </td>
                        <td>
                            <input type="text" name="bulk_payment_amount[]" class="form-control  bulk_payment_amount " value=""  >
                        </td>
                        <td>
                            <input type="text" name="bulk_supplier_bank_id[]" class="form-control  bulk_supplier_bank_id " value="" readonly >
                        </td> 
                         <td>
                            <input type="text" name="bulk_supplier_account_name[]" class="form-control  bulk_supplier_account_name " value="" readonly >
                        </td> 
                         <td>
                            <input type="text" name="bulk_supplier_account_no[]" class="form-control  bulk_supplier_account_no " value="" readonly >
                        </td> 
                         <td>
                            <input type="text" name="bulk_supplier_ifsc_code[]" class="form-control  bulk_supplier_ifsc_code " value="" readonly >
                        </td> 
...
$(document).on('change', '.bulk_employee_id', function () {
    ...
    if (!employee_id) {
        $row.find('.bulk_supplier_bank_id').val('');
        $row.find('.bulk_supplier_account_name').val('');
        $row.find('.bulk_supplier_account_no').val('');
        $row.find('.bulk_supplier_ifsc_code').val('');
        return;
    }

    let url = "{{ URL::to('getemployeebankdetails') }}/" + employee_id;

    $.get(url, function (data) {
        if (data.length > 0) {
            $row.find('.bulk_supplier_bank_id').val(data[0].bank_name);
            $row.find('.bulk_supplier_account_name').val(data[0].account_holder_name);
            $row.find('.bulk_supplier_account_no').val(data[0].account_number);
            $row.find('.bulk_supplier_ifsc_code').val(data[0].ifsc_code);
        }
    });
});
