...
<?php include('tools_menu.php');?> 
 <h4 class="heads">Payment For Invoice<span class="ui_close_btn"><a class="collapse-close pull-right btn-danger" onclick="location.href = '{{url('paymentforinvoice')}}'"></a></span>
 </h4>
    ...
    <span class="ui_close_btn"></span>
      ...
      <input type="hidden" class="form-control po_hdr_id" id="po_hdr_id" name="po_hdr_id" size="16" value="{{ $row->po_hdr_id }}" hidden="true" style="display:none;" readonly>
                            ...
                            <div class="col-md-5">
                                <input type="text" id="tds_amount" name="tds_amount" class="form-control tds_amount" value="{{ $row->tds_amount }}" readonly>
                            </div>
                            <div class="col-md-2 showline">
                            </div>
                        </div>
  ...
  <div class="modal-dialog">
    ...
    font-size: 14px;">
               ...
               foreach($invoice_balamt as $k=>$v) { ?>  
                    <tr>
                  <td>{{$v->bill_number}}</td>
                  <td class="checkadv balance_amount balance_amount{{$i}}" id="{{$v->balance_amount}}">{{$v->balance_amount}}</td>
  ...
  <div class="modal-dialog">
    ...
    font-size: 14px;">
               ...
               foreach($po_balamt as $k=>$v) { ?>  
                  ...
                  <td><input type="text" name="popaymentamt[{{$v->po_hdr_id}}]" class="popaymentamt"></td>
