...
$pdf = new MYPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
    ...
    $pdf->Cell(0,0,$company_name,0,1,'L');
		...
		$emp_login_id = \Session::get('emp_id');
		$dept_login_id = \Session::get('dept_id');
		$deptid_login=json_decode($dept_login_id);
		//dd($deptid_login);
        //dd(\Session::all());
        if($emp_details[0]->employee_id == $emp_login_id || in_array("28", $deptid_login )){
...
//*********************** DECIDINGE THE PAGE ERROR OR CONTENT****////
			...
			$pdf->SetFont('','B','15');
			$pdf->SetTextColor('0','0','0');
			$pdf->MultiCell(120,10,"You don't have access to view other's payslip",'0','C');
}
