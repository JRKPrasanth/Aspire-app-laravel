...
@section('content')
  <h3 class="text-danger">Material Bom Upload</h3>
  ...
  <div class="card shadow-lg rounded-4 border-0">
    ...
    <div class="card-body">
      <form method="post" action="" id="materialbomupload" enctype="multipart/form-data">
        {{ @csrf_field() }}
        ...
        <div class="row g-3 align-items-center mb-3">
          ...
          <div class="col-md-3">
            <a href="{{ url('Uploads/MATERIAL BOM TEMPLATE.csv') }}" class="btn btn-outline-primary w-100" download>
...
@push('scripts')
  ...
  <script>
    ...
    $.ajax({
      ...
      type: "GET",
      data: {
        table: "w_bomupload_t:batch_name:batch_name",
        parent: condition1,
    ...
    $(document).ready(function () {
      ...
      var table = $('#BomTbl').DataTable({
        ...
        columns: [
          ...
          { data: 'bom_product', name: 'bom_product' },
          { data: 'process_level', name: 'process_level' },
    ...
    $(document).on('click', '.edit-btn', function () {
      ...
      if (status == "ERROR") {
        window.location.replace('materialbomuploadedit/' + id);
      }
      else {
    ...
    $('#myModal1').on('shown.bs.modal', function () {
      $('.index').click(function () {
        ...
        if (tmp2 != "") {
          var temp = tmp1 + tmp2;
          $('.batch_name').val(temp);
          var form_data = new FormData(document.getElementById('materialbomupload'));
          var $btn = $(this);
          $btn.prop('disabled', true);
          $.ajax({
            url: "{{URL::to('materialbomuploaddata')}}",
            data: form_data,
            type: 'POST',
