...
@section('content')
    <h3 class="text-danger">Zone and Product wise Target and sale</h3>
    ...
    <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
        <form action="{{ url('primarysaleszonepro') }}" method="get" id="searchForm">
            <div class="row g-3">
                <div class="col-md-3">
            ...
            <table id="Table1" class="table table-bordered table-striped table-hover w-100">
                <thead>
                    <tr>
                        ...
                        <th colspan="8" class="align text-white bg-danger text-center">Zone Wise Product Target and
                            Achivement upto - {{ $mon_yr }}</th>
                    ...
                    <tr>
                        ...
                        <th class="align  bg-warning text-center">Division</th>
                        <th class="align  bg-warning text-center">Product Name</th>
    ...
    foreach ($all_ind_sal as $value) {
        ...
        if (!isset($productZoneData[$Zone])) {
            $productZoneData[$Zone] = [];
        }

        if (!isset($productZoneData[$Zone][$Product])) {
            $productZoneData[$Zone][$Product] = [
        ...
        if ($value->f_year == $pre_fy_year) {
            $productZoneData[$Zone][$Product]['primary_dis_pre_fy'] += $value->sale;
        } elseif ($value->f_year == $cur_fy_year) {
            $productZoneData[$Zone][$Product]['primary_dis_cur_fy'] += $value->sale;
            $productZoneData[$Zone][$Product]['target_cur_fy'] += $value->target;
        }
    ...
    foreach ($productZoneData as $zone => $products) {
        foreach ($products as $product => $data) {
    ?>
                    <tr>
                        <td><?php        echo $data['zone']; ?></td>
                        <td><?php        echo $data['state']; ?></td>
                        <td class="sticky-col"><?php        echo $data['product']; ?></td>
...
@push('scripts')
    ...
    <script>
        ...
        document.getElementById('searchForm').addEventListener('submit', function (event) {
            ...
            if (zone === '' && region === '' && startDate === '' && Division === '' && dist_name === '') {
