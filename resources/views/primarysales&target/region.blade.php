...
function getBackgroundColor($value1, $value2) {
    if ($value2 == 0) {
        return 'background: #c96fc6;'; // Default/failure color if denominator is zero
    }
    ...
    if ($percentage >= 0 && $percentage <= 50) {
        ...
        return 'background: #0bb921;';
    } else {
        return ''; // No background if none of the conditions are met
    }
}
?>
...
<div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
  <form action="{{ url('primarysalesandtargetregion') }}" method="get" id="searchForm">
    ...
    <div class="table-responsive" style="overflow-x: auto;">
        <table id="Table1" class="table table-bordered table-striped table-hover w-100">
            <thead>
                <tr>
                    <?php if (request('zone') != ''): ?>
                        <th class="align" style="background:#fdf15c">{{ request('zone') }}</th>
                        <?php endif; ?>
                    <?php if (request('region') != ''): ?>
                        <th class="align" style="background:#fdf15c">{{ request('region') }}</th>
                        <?php endif; ?>
                        <th colspan="6" class="align text-white bg-danger text-center" >REGION WISE SALES AND TARGET UPTO {{ $mon_yr }}</th>
                    
                </tr>
                    <tr class="text-white bg-secondary text-center">
					  <th colspan="1" class="text-white bg-secondary text-center"></th>
					  <th colspan="1" class="text-white bg-secondary text-center">Sale</th>
					   <th colspan="1" class="text-white bg-secondary text-center">Sale</th>
					  <th colspan="1" class="text-white bg-secondary text-center" >Target</th>
					   <th colspan="2" class="text-white bg-secondary text-center"></th>
    
                    </tr>

  <tr>
                        <th class="align bg-warning text-center ">Region</th>
                     <th class="align bg-warning text-center">{{ $pre_fy_year }}</th>
                    <th class="align bg-warning text-center">{{ $cur_fy_year}}</th>
                    <th class="align bg-warning text-center">{{ $cur_fy_year}}</th>
    ...
    <td class="rupee-value" data-original="<?php echo $data['primary_dis_pre_fy']; ?>" style="text-align: center;"><?php echo $data['primary_dis_pre_fy']; ?></td>
    <td class="rupee-value" data-original="<?php echo $data['primary_dis_cur_fy']; ?>" style="text-align: center;"><?php echo $data['primary_dis_cur_fy']; ?></td>
    <td class="rupee-value" data-original="<?php echo $data['target_cur_fy']; ?>" style="text-align: center;"><?php echo $data['target_cur_fy']; ?></td>
      <?php  if ($data['primary_dis_pre_fy'] != 0) { ?>    <td style="text-align: center;"><?php echo round(($data['primary_dis_cur_fy'] / $data['primary_dis_pre_fy'] - 1) * 100, 0); ?>%</td> <?php }else{ ?>  <td style="text-align: center;">0</td> <?php } ?>
    ...
    </td> <?php }else{ ?>  <td style="text-align: center;background: #c96fc6;">0</td> <?php } ?>
    </tr>
    <?php } ?>
      </tbody>
    <!-- Add the total row for all products -->
  <tfoot>
    <tr class="sticky-foot table-danger fw-bold">
    ...
    <td class="rupee-value" data-original="<?php echo $divisionTotals['totaltarget_cur_fy']; ?>" ><?php echo $divisionTotals['totaltarget_cur_fy']; ?></td>
         <?php  if ($divisionTotals['totalPrimaryDis_pre_fy'] != 0) { ?>    <td><?php echo round(($divisionTotals['totalPrimaryDis_cur_fy'] / $divisionTotals['totalPrimaryDis_pre_fy'] - 1) * 100, 0); ?>%</td><?php }else{ ?>  <td>0</td> <?php } ?>
     ...
     <!-- END -->
         ...
         <!-- machine wise summary chart -->
             <div class="card shadow-lg rounded-4 border-0 p-3 mb-4">
                <h5 class="chart_tittle">Region Wise Growth And Achivement UPTO - {{ $mon_yr }}</h5>
    ...
    <script>
        ...
        fixedHeader: true,
            ...
            buttons: [
                {
                    extend: 'excelHtml5',
                    filename: 'primary_sale_and_target_region',
                    exportOptions: {
                        format: {
...
new Chart("myChart", {
    ...
    options: {
        ...
        title: {
            display: true,
            text: "Region Growth and Achievement Chart"
        },
        tooltips: {
            callbacks: {
...
// Example using jQuery (you probably already use it in your system):

$('body').on('click', '#sidebar', function() {
    setTimeout(function() {
        $('#Table1').DataTable().columns.adjust().draw();
    }, 300); // wait a bit after animation
});
