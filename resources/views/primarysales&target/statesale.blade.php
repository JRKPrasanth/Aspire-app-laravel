...
@section('content')
<h3 class="text-danger">State wise sale with month wise</h3>
...
<div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
    <form action="{{ url('primarysalesstockdts') }}" method="get" id="searchForm">
        <div class="row g-3">
            <div class="col-md-3">
...
<div class="card shadow-lg rounded-4 border-0 p-4">
            ...
            <tr>
              ...
              <th class="align text-white bg-danger text-center">{{ request('state') }}</th>
                <?php endif; ?>
                  ...
                  <?php endif; ?>
                <th colspan="15" class="align text-white bg-danger text-center">STATE WISE SALE WITH MONTH WISE UPTO - {{ $mon_yr }}</th>
            </tr>
            <tr>
    ...
    foreach ($all_ind_sal as $value) {
        ...
        $monthlySales[$value->state][$value->f_year][$value->month] = $value->sale;
    }
    ?>

<tbody>
    <?php
    foreach ($monthlySales as $fYear => $states) {
                    foreach ($states as $state => $monthData) {
                        echo '<tr>';
                        echo '<td class="sticky-col" >' . $fYear . '</td>';
                        echo '<td   >' . $state . '</td>';
