...
@section('content')
<h3 class="text-danger">State Wise</h3>
...
function getBackgroundColor($value1, $value2) {
    ...
    if ($percentage >= 0 && $percentage <= 50) {
        ...
        return ''; // Default background or no background if none of the conditions are met
    }
}
?>
...
<div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
  <form action="{{ url('primarysalesandtargetstate') }}" method="get" id="searchForm">
...
<div class="card shadow-lg rounded-4 border-0 p-4">
 ...
 <thead>
            <tr>
                <?php if (request('zone') != ''): ?>
                    <th class="align">{{ request('zone') }}</th>
                    <?php endif; ?>
                <?php if (request('region') != ''): ?>
                    <th class="align">{{ request('region') }}</th>
                    <?php endif; ?>
                    <th colspan="7" class="align text-white bg-danger" >STATE WISE SALES AND TARGET UPTO {{ $mon_yr }}</th>
  ...
  <tbody>
            ...
            foreach ($all_ind_sal as $value) {
                ...
                if ($value->f_year == $pre_fy_year) {
                    ...
                    $zoneData[$Zone][$Region]['target_cur_fy'] += $value->target;
                }
                ...
                foreach ($regions as $Region => $data) {
                    ...
                    <tr>
                        ...
                        <?php if ($data['target_cur_fy'] != 0) { ?>
                            <td style=" text-align: center; <?php echo getBackgroundColor($data['primary_dis_cur_fy'], $data['target_cur_fy']); ?>">
        ...
        <!-- Add the total row for all products -->
            <tfoot>
            <tr class="sticky-foot  table-danger text-center fw-bold">
                ...
                <?php if ($divisionTotals['totaltarget_cur_fy'] != 0) { ?>
                    <td style=" text-align: center; <?php echo getBackgroundColor($divisionTotals['totalPrimaryDis_cur_fy'], $divisionTotals['totaltarget_cur_fy']); ?>">
     ...
     <!-- END -->
         <!-- machine wise summary chart -->
             <div class="card shadow-lg rounded-4 border-0 p-3 mb-4">
                    <h5 class="chart_tittle">State Wise Growth And Achivement UPTO - {{ $mon_yr }}</h5>
    ...
    <script>
        ...
        $('#Table1').DataTable({
            ...
            buttons: [
                {
                    extend: 'excelHtml5',
                    filename: 'primary_sale_and_target_state',
                    exportOptions: {
                        format: {
...
new Chart("myChart", {
    ...
    options: {
        ...
        title: {
            display: true,
            text: "State Growth and Achievement Chart"
        },
        tooltips: {
            callbacks: {
