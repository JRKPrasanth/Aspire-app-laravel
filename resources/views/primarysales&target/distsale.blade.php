...
@section('content')
    <h3 class="text-danger">Distributor wise sale with month wise</h3>
    ...
    <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
        <form action="{{ url('primarysalesdistwise') }}" method="get" id="searchForm">
            <div class="row g-3">
                <div class="col-md-3">
                ...
                <thead>
                    <tr>
                        ...
                        <th class="align text-white bg-danger text-center" {{ request('area') }}</th>
                            <?php endif; ?>
                        <th colspan="15" class="align text-white bg-danger text-center">DISTRIBUTOR WISE SALE WITH MONTH
                            WISE UPTO - {{ $mon_yr }}</th>
                    </tr>
                    <tr>
    ...
    foreach ($all_ind_sal as $value) {
        ...
        $monthlySales[$value->name][$value->f_year][$value->state][$value->month] = $value->sale;
    }
        ?>
    ...
    foreach ($monthlySales as $name => $financialYears) {
        foreach ($financialYears as $fYear => $states) {
