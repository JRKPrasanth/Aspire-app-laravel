...
<h2 class="heads">Stockist DCR  
  <span class="ui_close_btn"><a href="../stockistdcr" class="collapse-close pull-right btn-danger" ></a></span>
</h2>
...
<form method="post" action="{{URL::to('stockistdcrsave')}}" id="stockistdcr" class="stockistdcr"  enctype="multipart/form-data">
	 {{ csrf_field() }}
...
<div class="col-md-12">
    ...
    <div class="col-md-4">
        ...
        <div class="form-group row">
            ...
            <div class="col-md-6">
            <input class="form-control stockist_dcr_id" id="stockist_dcr_id" name="stockist_dcr_id" size="16" type="hidden" value="{{ $row->stockist_dcr_id }}" readonly>
  ...
  <div class="col-md-4">
    <div class="form-group row">
            <label for="inputIsValid" class="form-control-label col-md-5"><span style="color: red;" >*</span>Stockist Name</label>
            <div class="col-md-6">
                <select name='stockist_name' class='form-control select2 stockist_name' id="stockist_name" required >
                    {!! $stockist_name !!}
                </select>
            </div>
        </div>
    ...
    <div class="col-md-12">
        ...
        <table class="overflow-y preview pob_qty">
            ...
            <?php if (count($linepob) >= 1) { ?>
                @foreach($linepob as $k => $v)
                    <tr class="clone rcopy3">
                        <td>
                            <input type="hidden" name="bulk_stockist_detail_id[]" class="form-control input-sm bulk_stockist_detail_id" value="{{ $v->stockist_detail_id }}">
                        </td>
                        <td>
            ...
            <?php } if (count($linepob) < 1) { ?>
                <tr class="clone rcopy3">
                    <td>
                        <input type="hidden" name="bulk_stockist_detail_id[]" class="form-control input-sm bulk_stockist_detail_id" >
                    </td>
                    <td>
                        <select class="bulk_product_order select2 form-control" id="bulk_product_order" name="bulk_product_order[]" required >
    	...
    	<div class="row">
    		<div class="col-lg-12 col-md-12">
    			<div class="form-group text-center">
    				...
    				<a class='btn cancel' onclick="location.href = '{{url::to('stockistdcr')}}'">Cancel</a>
...
@media screen and (min-width: 1500px) {
    .bulk_product_order{width: 450px !important;}
    .bulk_pob_qty{width: 450px !important;}
}
...
$(document).ready(function(){
    ...
    $(document).on('change','#tp_date',function(){
        ...
        var id = $('#stockist_dcr_id').val();
        if(id==''){
            id = 0;
        }
        var url = "{{ URL::to('stockistareas')}}/"+date+"/"+id;
    ...
    $(document).on('change','#from_area',function(){
        ...
        var url = "{{ URL::to('stockistdetail')}}/"+id;
        $.get(url,function(data){
            $(".stockist_name").html(data);
        });
    });
	...
	$(document).on('click','.saveform',function()
    {
    	...
    	var url			="{{ url('stockistdcrsave') }}";

        var red_url		="{{ url('stockistdcr') }}";

        var create_url	="{{ url('stockistdcrcreate') }}/0";
        

        validationrule('stockistdcr');
        change_date();
        var formdata	= $('#stockistdcr').serialize();
        var form = $('#stockistdcr');
    ...
    $(document).on('click', '.removepob', function ()
    {
        ...
        if(rowCount > 1 ){
            ...
            removeClass('bulk_stockist_detail_id');
        }else{
...
function changeclassfields(){
    ...
    changeClassName('bulk_stockist_detail_id');
}
