...
@section('content')
<h3 class="text-danger">Qualit Indent</h3>
...
<script>
    ...
    var table = $('#QcTbl').DataTable({
      processing: true,
      serverSide: true,
      ajax: "{{ route('qualityindentdata') }}",
      columns: [
        { data: 'indent_name', name: 'indent_name' },
        { data: 'indent_date', name: 'indent_date' },
        { data: 'remarks', name: 'remarks' },
        {
          data: 'quality_indent_hdr_id',
          name: 'actions',
          orderable: false,
          searchable: false,
...
{
  var url="{{ URL::to('qualityindentcreate/0')}}";
    window.location.replace(url);
});
	//edit function
    $(document).on('click', '.edit-btn', function () {
    const id = $(this).data('id');
    const url = "{{ url('qualityindentcreate') }}/" + id;
