...
class MYPDF extends TCPDF
{
    ...
    public function Header()
    {
        ...
        //dd($image_file);
        $this->Image($image_file, 12, 7, 20, 15, 'JPG', '', 'T', false, 250, '', false, false, 0, false, false, false);
        // Set font
    ...
    public function Footer()
    {
        ...
        // Page number
        $this->Cell(0, 10, 'This is a computer generated invoice', 0, false, 'C', 0, '', 0, false, 'T', 'M');
...
$pdf->SetFont('', '', '8');
$pdf->SetTextColor('0', '0', '0');
$pdf->MultiCell(100, 2, $bill_to_address_1, 0, 'L');
$oldad = $pdf->getY();
...
if ($invoice_type != "SAMPLE") {
    $pdf->SetXY(57, $oldad + 4);
    $pdf->SetFont('', 'B', '9');
    $pdf->SetTextColor('0', '0', '0');
    $pdf->MultiCell(100, 2, "GSTIN/UIN : ", 0, 'L');
    $pdf->SetXY(76, $oldad + 4);
    $pdf->SetFont('', '', '9');
    $pdf->SetTextColor('0', '0', '0');
    $pdf->MultiCell(100, 2, $ship_gst_no, 0, 'L');
}
$pdf->SetXY(13, $oldad + 4);
$pdf->SetFont('', '', '9');
$pdf->SetTextColor('0', '0', '0');
$pdf->MultiCell(100, 2, "State : " . $state_name_ship, 0, 'L');
...
$pdf->Line(12, 48, 109, 48, $style);
$pdf->Line(12, 75, 109, 75, $style);
$pdf->Line(109, 59, 208, 59, $style);
$pdf->Line(160, 22, 160, 68, $style);
/************************************Header bottom line******************************/
...
if ($pdf->pageNo() == $total) {
    if ($oldY < 175) {
        ...
        $pdf->SetFont('', '', '8');
        $pdf->SetTextColor('0', '0', '0');
        $pdf->MultiCell(14, 2, number_format($freeqtytotal, '2', '.', ''), 0, 'R');
       ...
       // dd($ta);
        ...
        if (count($gst) > 0) {
            ...
            if ($words == "GST") {
                ...
                $pdf->SetTextColor('0', '0', '0');
                //$pdf->Cell(0,0,$ka ,0,1,'L');
                $pdf->MultiCell(158, 2, number_format($tot_tax_amt, '2', '.', ''), 0, 'R');
            }
        }
        ...
        if ($tcs_applicable == "YES") {
            $pdf->Line(180, 208, 208, 208, $style);
            $pdf->Line(180, 212, 208, 212, $style);
        } else {
...
if($trade_discount_pre=0)
{
        ...
        if (count($gst) > 0) {
            ...
            if ($words == "GST") {
                ...
                $pdf->SetTextColor('0', '0', '0');
                //$pdf->Cell(0,0,$ka ,0,1,'L');
                $pdf->MultiCell(152, 2, number_format($tot_tax_amt, '2', '.', ''), 0, 'L');
            }
        }
        if ($tcs_applicable == "YES") {
            $pdf->SetXY(70, 204);
