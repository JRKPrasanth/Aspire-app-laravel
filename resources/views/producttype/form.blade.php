...
@section('content')
    <h3 class="text-danger">Product Type</h3>
    ...
    <div class="card shadow-lg rounded-4 border-0">
        ...
        <div class="card-body">
            <form id="prdtypesave" method="post" action="" data-parsley-validate>
                <?php $data = \Session::get('data');
    if (isset($data[$pageMethod]['save'])) { ?>
                {{ csrf_field() }}
                <input type="hidden" value="" name="savestatus" id="savestatus" />
                <input type="hidden" name="edit_id" value="{{ $row->product_type_id }}" id="edit_id" />
                        ...
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-md-7">
                                <input type="text" name="product_type" id="product_type" value=""
                                    class="form-control product_type" required tabindex="1">
...
@push('scripts')
    ...
    <script>
                ...
                columns: [
                  ...
                  <i class="bi bi-pencil"></i>
                            ...
                            if (window.toolbarButtons?.some(btn => btn.attr.id === 'delete')) {
                                buttons += `
                <button class="btn btn-sm btn-danger delete-btn" data-id="${row.product_type_id}">
        ...
        function duplicate_validate() {
            ...
            var url = "{{URL::to('producttypecheckname')}}"
            ...
            $.ajax({
                ...
                dataType: 'json',
                async: false,
                data: { product_type: product_type, edit_id: edit_id },
                success: function (response) {
                    if (response == 1) {
                        $('.dup_name').html('Product type:' + product_type + ' Already Exists ');
                        $('.dup_name').show();
                        $(".product_type").val('');
                        dup_chk = false;
                    }
        ...
        $(document).on('click', '.saveform', function () {
            ...
            if (form.parsley().isValid()) {
                ...
                var url = "{{ URL::to('producttypesave')}}";
                if (dup_chk == true) {
        ...
        $(document).on('click', '.edit-btn', function () {
            ...
            $.get(url, function (data) {
                ...
                if (data == 1) {
                    ...
                    showCustomAlert("Product Type Already Used In Some Where.Unable to Edit", 'warning');
                    ...
                    $('input[name="product_type"]').val(name);
        ...
        $('#globalConfirmDeleteBtn').on('click', function () {
            if (deleteId) {
                $.ajax({
                    url: "{{ url('producttypedelete') }}/" + deleteId,
                    type: "GET",
