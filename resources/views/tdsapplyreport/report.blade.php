...
@section('content')
    <h3 class="text-danger">TDS Applicable Report</h3>
    @include('layouts.breadcrumb')

    <div class="card shadow-lg rounded-4 border-0">
        <div class="card-body p-4">
            <form action="{{ url('tdsapplyreport') }}" method="get" id="searchForm">
                @csrf
    ...
    <div class="card shadow-lg rounded-4 border-0 p-4">
        <div class="table-responsive" style="overflow-x: auto;">
            <table id="Table1" class="table table-bordered table-striped table-hover w-100">
                <thead>
                    <tr>
                        <th class="text-center text-white bg-danger">Supplier Name</th>
                        <th class="text-center text-white bg-danger">Zone</th>
                        <th class="text-center text-white bg-danger">Invoice Grand Total</th>
                        <th class="text-center text-white bg-danger">TDS Applicable</th>
                </thead>
                <tbody>
                    <?php foreach ($tds_apply as $value) { ?>
                    <tr>
                        <td>{{$value->supplier_name}}</td>
                        <td>{{$value->zone}}</td>
                        <td>{{ number_format($value->total, 2) }}</td>
                        <td>{{ number_format($value->tds_apply_amt, 2) }}</td>
