...
$( document ).ready(function() {
      ...
      mtype: "GET",
    colModel: [
    { name: "doctor_id", label: "id" ,hidden:true},
{ name: "doctor_name", label: "doctor name" ,editable:true,key: true, editrules:{date:true}},
{ name: "doctor_phone_number", label: "Phone Number" ,editable:true, editrules:{date:true}},
{ name: "gender", label: "Gender" ,editable:true, editrules:{date:true}},
{ name: "doctor_email_id", label: "Email Id",editable:true, editrules:{date:true}},
...
$("#edit").click(function()
{
	...
	var cellValue = jQuery("#grid1").jqGrid ('getCell', gr, 'doctor_id');
  var type = "<?php echo $urlname ?>";

	if(cellValue)
	{
    var url = "{{ url('doctoredit') }}/";
...
$("#delete").click(function(){
  ...
  if( cellValue )
  {
    ...
    if(e == true)
      {
        var url ="{{ URL::to('doctordelete') }}/" +cellValue;
        $.get(url,function(data)
        {
          var data = $.trim(data);
          var red_url ="{{ URL::to('doctor') }}";
...
$(".jqgrow.ui-row-ltr ui-widget-content").attr("id","");
  </script>
@endsection
