...
@section('content')
  <h3 class="text-danger">State Wise Ratio</h3>
  ...
  <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
    <form action="{{ url('primarysalestaratio') }}" method="get" id="searchForm">
  ...
  <div class="card shadow-lg rounded-4 border-0 p-4">
      ...
      <table id="Table1" class="table table-bordered table-striped table-hover w-100">
        <thead>
          <tr>
            ...
            <th colspan="68" class="align text-white bg-danger text-left">ALL INDIA - % SAMPLE to SALES RATIO - STATE WISE
            </th>
          </tr>
          <tr>
            <th colspan="1" class="text-text bg-warning text-center">Sum of Asseesable Value</th>
            <?php $Zones = []; ?>
            <?php foreach ($zone_wise as $value) {
    $zone_type = $value->state;
    if (!in_array($zone_type, $Zones)) {
      $Zones[] = $zone_type; ?>
          ...
          <tr>
            <th class="text-white bg-secondary text-center">Product</th>
            <?php foreach ($Zones as $zone) { ?>
            <th class="bg-warning">Sales</th>
            <th class="bg-warning">Sample</th>
  ...
  foreach ($products as $product): ?>
        ...
        return $value->state == $zone && $value->sfg_product_name == $product;
      });
      ...
      if (!empty($productData)):
        $value = reset($productData); // Get the first element
            ...
            <td>
              <?php      echo ($value->sale != '0') ? number_format($value->sample / $value->sale * 100, 0) . '%' : '0'; ?>
            </td>
            <?php    else: ?>
            <td>0</td>
            <td>0</td>
