...
@section('content')
  <h3 class="text-danger">Person Wise Sample</h3>
  ...
  <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
    <form action="{{ url('primarypersonsample') }}" method="get" id="searchForm">
      <div class="row g-3 align-items-end">
        ...
        <div class="col-md-3">
          ...
          <input class="form-control date_select" id="date_select" name="date_select" type="month" autocomplete="off">
        </div>
      ...
      <table id="Table1" class="table table-bordered table-striped table-hover w-100">
        <thead>
          <tr>
            ...
            <th colspan="2" class="align text-white bg-danger text-center">last 6 months upto {{ $mon_yr}}</th>
            <th colspan="7" class="align text-white bg-danger text-center">PERSON WISE SAMPLED</th>
          ...
          <tr>
            ...
            <th class="align text-white bg-secondary text-center">Person Name</th>
  ...
  foreach ($month_sample as $value) {
    ...
    $monthlySales[$value->name][$value->state][$value->sfg_product_name][$value->month_y] = $value->sample;
  }

  // Loop through unique F_Year, HQ Name, Zone, Region, State combinations
  foreach ($monthlySales as $name => $states) {
    foreach ($states as $state => $sfg_product_names) {
      foreach ($sfg_product_names as $sfg_product_name => $monthData) {

        echo '<tr>';
        echo '<td >' . $name . '</td>';
        echo '<td class="sticky-col"  >' . $state . '</td>';
        echo '<td >' . $sfg_product_name . '</td>';
...
@push('scripts')
  ...
  <script>
    ...
    document.getElementById('searchForm').addEventListener('submit', function (event) {
      ...
      if (zone === '' && region === '' && startDate === '' && Person === '' && state === '') {
