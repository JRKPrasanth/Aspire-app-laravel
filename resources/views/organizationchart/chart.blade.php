...
#svgChart{ min-width:0!important; min-height:0!important; }
...
.node-group .emp-count-icon {
  ...
  <div class="card shadow-lg rounded-4 border-0 container mt-4">
    <div class="row align-items-end mb-3 mt-4">
        @php error_reporting(0); @endphp
        @php $dashboardTitleDisplayed = false; @endphp

        @foreach ($das_access as $val)
            @if ($val == 'OrganogramSearch')
                <div class="col-lg-4 mb-2">
                    <form action="{{ url('companyorganogram') }}" method="get" id="searchForm">
                        <label for="emp_search" class="form-label">Select Zone</label>
                        <select name="emp_search" id="emp_search" class="form-select select2">
                          <option value="">-- Please select --</option>
                          
                          <?php if (\Session::get('groupname') == '16') { ?>
                            <option value="3">East</option>
                            <option value="4">Central West</option>
                            <option value="5">North</option>
                            <option value="6">South-1</option>
                            <option value="7">South-2</option>
                            <option value="8">South-3</option>
                            <option value="9">Corporate</option>
                          <?php  } else { ?>
                            <option value="1">HO/CO</option>
                             <option value="3">East</option>
                            <option value="4">Central West</option>
                            <option value="5">North</option>
                            <option value="6">South-1</option>
                ...
                @php $dashboardTitleDisplayed = true; break; @endphp
            @endif
        @endforeach

        <div class="col-lg-2 mb-2">
            <button type="button" class="btn btn-outline-primary w-100" onclick="params.funcs.expandAll()">Expand All</button>
        </div>
    </div>
 </div>
	  
    <div class="card shadow-sm rounded-4 mb-3" style='display: none;'>
            ...
            <div class="user-search-box">
                <div class="input-box position-relative">
                    ...
                    <div class="input-wrapper">
                        ...
                        <div class="form-text">By Firstname, Lastname, Tags</div>
                    </div>
                </div>
...
@push('scripts')
  ...
  <script src='https://cdnjs.cloudflare.com/ajax/libs/d3/3.5.17/d3.min.js'></script>
    ...
    var Url = "{{ route('companyorganogramdata') }}";
...
collapsiblesWrapper.append("text")
  ...
  .style('x', '234px') // Set x position using style
  .style('y', '85px')   // Set y position using style
  .text(function(d) {
...
collapsiblesWrapper.on("click", click);
    ...
    function tooltipContent(item) {
      ...
      strVar += "      <p class=\"position\">" + item.area + " <\/p>";
      strVar += "";
      strVar += "      <p class=\"position\">" + "Area: " + item.place + "<\/p>";
      strVar += "      <p class=\"position\">" + "DOJ: " + item.doj + "<\/p>";
      strVar += "      <p class=\"position\">" + "Grade: " + item.grade + "<\/p>";
      strVar += "      <p class=\"deptbtn btn-tooltip-department'\">" + item.positionName + " <\/p>";
      strVar += "  <\/div>";
      strVar += "";
  ...
  function reflectResults(results) {
    var htmlStringArray = results.map(function(result) {
      ...
      strVar += "               <p class=\"position-name\">" + item.positionName + "<\/p>";
...
function expandAll() {
  ...
  function getAll(selector) {
    ...
    $(document).ready(function () {
        var empId = "{{ request('emp_search') }}";

        $('.emp_search').select2();
        $('#emp_search').val(empId).trigger('change');
...
function setManualCanvas(w, h){
  const svg = document.querySelector("#svgChart svg");
  if(!svg) return;
  svg.setAttribute("width", w);
  svg.setAttribute("height", h);
}

setTimeout(()=> setManualCanvas(3500, 2200), 300);
