...
@section('content')
    <h3 class="text-danger">Distributor Wise</h3>
    ...
    <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
        <form action="{{ url('globalmisreportdistributor') }}" method="get" id="searchForm">
            <div class="row g-3">
            ...
            <table id="Table1" class="table table-bordered table-striped table-hover w-100">
                <thead>
                    <tr>
                        ...
                        <?php endif; ?>
                        <th colspan="5" class="align text-white bg-danger text-center">DISTRIBUTOR WISE SALES AND GROWTH
                            UPTO {{ $mon_yr }}</th>
                    ...
                    <tr>
                        ...
                        <th class="align text-white bg-success text-center ">Distributor Names</th>
                        ...
                        <th class="align text-dark bg-warning text-center">Growth</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
    ...
    foreach ($all_ind_sal as $value) {
        ...
        if (!isset($zoneData[$Zone][$Region])) {
            $zoneData[$Zone][$Region] = [
                'state' => $value->state,
        ...
        if ($value->f_year == $pre_fy_year) {
            $zoneData[$Zone][$Region]['primary_dis_pre_fy'] += $value->sale;
        } elseif ($value->f_year == $cur_fy_year) {
            $zoneData[$Zone][$Region]['primary_dis_cur_fy'] += $value->sale;
        }
    ...
    foreach ($zoneData as $Zone => $regions) {
        foreach ($regions as $Region => $data) {
                        ?>
                    <tr>
                        <td class="sticky-col"><?php        echo $data['state']; ?></td>
            ...
            <table id="Table2" class="table table-bordered table-striped table-hover w-100">
                <thead>
                    <tr>
                        ...
                        <th class="align text-white bg-danger text-center">{{ request('region') }}</th>
                        <?php endif; ?>
                        <th colspan="5" class="align text-white bg-danger text-center">DISTRIBUTOR WISE SALES UNIT AND
                            GROWTH UPTO {{ $mon_yr }}</th>
                    ...
                    <tr>
                        ...
                        <th class="align text-white bg-success text-center">{{ $pre_fy_year }}</th>
                        <th class="align text-white bg-success text-center">{{ $cur_fy_year}}</th>
    ...
    foreach ($all_ind_sal_product as $value) {
        $Zone = $value->product;
        $Region = $value->name;
    ...
    foreach ($zoneData as $Zone => $regions) {
        foreach ($regions as $Region => $data) {
                        ?>
                    <tr>
                        <td style="text-align:center;"><?php        echo $Region; ?></td>
                        <td class="sticky-col" style="text-align:center;"><?php        echo $data['state']; ?></td>
                        <td class="rupee-value" data-original="<?php        echo $data['primary_dis_pre_fy']; ?>"
                            style="text-align: center;"><?php        echo $data['primary_dis_pre_fy']; ?></td>
                        <td class="rupee-value" data-original="<?php        echo $data['primary_dis_cur_fy']; ?>"
                            style="text-align: center;"><?php        echo $data['primary_dis_cur_fy']; ?></td>
        ...
        $(document).ready(function () {
            $('#Table1').DataTable({
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        filename: 'DISTRIBUTOR WISE SALES AND GROWTH',
                        exportOptions: {
                            format: {
            ...
            $('#Table2').DataTable({
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        filename: 'DISTRIBUTOR WISE SALES UNIT AND GROWTH',
                        exportOptions: {
                            format: {
                                header: function (data, columnIdx) {
