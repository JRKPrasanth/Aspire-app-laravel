...
@section('content')
  ...
  <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
    <form action="{{ url('primarysalesreturnregionratio') }}" method="get" id="searchForm">
      ...
      <table id="Table1" class="table table-bordered table-striped table-hover w-100">
        <thead>
          <tr>
            ...
            <th colspan="39" class="align text-white bg-danger text-left">REGION WISE RATIO LAST 6 MONTHS UPTO -
              {{ $mon_yr }}</th>
          </tr>
          <tr>
            <th colspan="3" class="text-white bg-secondary text-left">Sum of Asseesable Value</th>
            <?php $Zones = []; ?>
            <?php foreach ($zone_wise as $value) {
    $zone_type = $value->region;
    if (!in_array($zone_type, $Zones)) {
      $Zones[] = $zone_type; ?>
            <th colspan="3" class="text-white bg-secondary text-left"><?php    echo $zone_type; ?></th>
            <?php  }
  } ?>
          </tr>
          <tr>
  ...
  foreach ($zones as $zoneColumn) {
    ...
    foreach ($Zones as $zoneKey) {
      ...
      foreach ($zone_wise as $value) {
        if ($value->sfg_product_name == $zoneColumn && $value->region == $zoneKey) {
    ...
    $(document).ready(function () {
      $('#Table1').DataTable({
        ...
        buttons: [
          {
            extend: 'excelHtml5',
            filename: 'region_wise_sales_return_ratio',
          },
          {
            extend: 'pdfHtml5',
            filename: 'region_wise_sales_return_ratio',
