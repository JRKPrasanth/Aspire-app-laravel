...
@section('content')
  <h3 class="text-danger">State Wise Return</h3>
  ...
  <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
    <form action="{{ url('primarysalesreturperstate') }}" method="get" id="searchForm">
      ...
      <table id="Table1" class="table table-bordered table-striped table-hover w-100">
        <thead>
          <tr>
            ...
            <th colspan="17" class="align text-white bg-danger text-center">STATE WISE SALES RETURN LAST 6 MONTHS UPTO -
              {{ $mon_yr }}</th>
          ...
          <tr>
            ...
            <th class="align sticky-col text-white bg-secondary text-center">Division</th>
            <th class="align sticky-coll text-white bg-secondary text-center">Product Name</th>
  ...
  foreach ($month_sample as $value) {
    ...
    $monthlySales[$value->division][$value->sfg_product_name][$value->state] = $value->sample;
  }
  ...
  foreach ($monthlySales as $division => $sfg_product_names) {
    foreach ($sfg_product_names as $sfg_product_name => $monthData) {
      ...
      echo '<td class="sticky-coll" >' . $sfg_product_name . '</td>';
    ...
    $(document).ready(function () {
      $('#Table1').DataTable({
        ...
        buttons: [
          {
            extend: 'excelHtml5',
            filename: 'state_wise_product_unit_return',
          },
          {
            extend: 'pdfHtml5',
            filename: 'state_wise_product_unit_return',
