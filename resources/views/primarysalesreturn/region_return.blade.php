...
@section('content')
  <h3 class="text-danger">Region Wise Return</h3>
  ...
  <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
    <form action="{{ url('primarysalesreturnsperregion') }}" method="get" id="searchForm">
      ...
      <table id="Table1" class="table table-bordered table-striped table-hover w-100">
        <thead>
          <tr>
            ...
            <th colspan="13" class="align text-white bg-danger text-center">REGION WISE SALES RETURN LAST 6 MONTHS UPTO -
              {{ $mon_yr }}</th>
  ...
  foreach ($month_sample as $value) {
    ...
    $monthlySales[$value->division][$value->sfg_product_name][$value->region] = $value->sample;
  }
    ...
    $(document).ready(function () {
      $('#Table1').DataTable({
        ...
        buttons: [
          {
            extend: 'excelHtml5',
            filename: 'region_wise_product_unit_return',
          },
          {
            extend: 'pdfHtml5',
            filename: 'region_wise_product_unit_return',
