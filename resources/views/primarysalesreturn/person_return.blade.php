...
@section('content')
  <h3 class="text-danger">Person Wise Return</h3>
  ...
  <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
    <form action="{{ url('primarysalesreturnsperperson') }}" method="get" id="searchForm">
      ...
      <table id="Table1" class="table table-bordered table-striped table-hover w-100">
        <thead>
          <tr>
            ...
            <th colspan="9" class="align text-white bg-danger text-center">STATEWISE DISTRIBUTOR WISE WITH MONTH WISE SALES RETURN LAST 6 MONTHS UPTO - {{ $mon_yr }}</th>
  ...
  foreach ($monthlySales as $state => $names) {
    foreach ($names as $name => $sfg_product_names) {
    ...
    $(document).ready(function () {
      $('#Table1').DataTable({
        ...
        buttons: [
          {
            extend: 'excelHtml5',
            filename: 'person_wise_product_unit_return',
          },
          {
            extend: 'pdfHtml5',
            filename: 'person_wise_product_unit_return',
