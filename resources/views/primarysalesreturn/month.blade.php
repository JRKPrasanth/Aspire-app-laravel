...
@section('content')
    ...
    <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
        <form action="{{ url('primarysalesreturn') }}" method="get" id="searchForm">
                    ...
                    <table id="Table1" class="table table-bordered table-striped table-hover w-100">
                        <thead>
                            <tr>
                                ...
                                <th colspan="1" class="align text-white bg-danger text-center">up to {{ $mon_yr}}</th>
                                <th colspan="1" class="align text-white bg-danger text-center">MONTH WISE</th>
                                <th colspan="2" class="align text-white bg-danger text-center">Sum of Asseesable Value</th>
                    ...
                    <table id="Table2" class="table table-bordered table-striped table-hover w-100">
                        <thead>
                            <tr>
                                ...
                                <th colspan="1" class="align text-white bg-danger text-center">up to {{ $mon_yr}}</th>
                                <th colspan="1" class="align text-white bg-danger text-center">ZONE WISE</th>
                                <th colspan="2" class="align text-white bg-danger text-center">Sum of Asseesable Value</th>
    ...
    <div class='row'>
        ...
        <div class='col-md-6 card shadow-lg rounded-4 border-0 p-4'>
            <h5 class="chart_tittle">Month Wise Sales Return Ratio% upto - {{ $mon_yr}}</h5>
            <canvas id="monthChart" style="max-height:460px"></canvas>

        </div>
        <div class='col-md-6 card shadow-lg rounded-4 border-0 p-4'>
            <h5 class="chart_tittle">Zone Wise Sales Return Ratio% upto - {{ $mon_yr}}</h5>
            <canvas id="zoneChart" style="max-height:460px"></canvas>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#Table1').DataTable({
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        filename: 'month_wise_sales_return',
                    },
                    {
                        extend: 'pdfHtml5',
                        filename: 'month_wise_sales_return',
        ...
        // charts 
        // Assuming $this->data['month_sumchart'] is a JSON-encoded string
        const productData = {!! $month_sumchart !!};

        var Months = productData.map(function (data) {
            return data.month;
        });

        var growthValues = productData.map(function (data) {
            return parseFloat(data.ratio.replace('%', '')) || 0;
        });
        ...
        const primchartData = {!! $month_sumchart !!};
        var variant = primchartData.map(item => item.month);
        ...
        new Chart("monthChart", {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: "Sales Return To Sale Ratio",
        ...
        const zoneData = {!! $zone_sumchart !!};
        var variant = zoneData.map(item => item.zone);
        var yValues = zoneData.map(item => parseFloat(item.ratio.replace('%', '')) || 0);
        var barColors = Array.from({ length: variant.length }, () => generateRandomColor());
        ...
        new Chart("zoneChart", {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: "Sales Return To Sale Ratio - Zone",
                        backgroundColor: barColors,
                        data: yValues
                    }
                ]
