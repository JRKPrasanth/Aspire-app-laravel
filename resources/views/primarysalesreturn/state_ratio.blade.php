...
@section('content')
  ...
  <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
    <form action="{{ url('primarysalesreturnstateratio') }}" method="get" id="searchForm">
      ...
      <table id="Table1" class="table table-bordered table-striped table-hover w-100">
        <thead>
          <tr>
            ...
            <th colspan="70" class="align text-white bg-danger text-center">STATE WISE RATIO LAST 6 MONTHS UPTO -
              {{ $mon_yr }}</th>
          </tr>
          <tr>
            <th colspan="2" class="text-white bg-secondary text-center">Sum of Asseesable Value</th>
            <?php $Zones = []; ?>
  ...
  foreach ($zones as $zoneColumn) {
    ...
    foreach ($Zones as $zoneKey) {
      ...
      foreach ($zone_wise as $value) {
        if ($value->sfg_product_name == $zoneColumn && $value->state == $zoneKey) {
    ...
    $(document).ready(function () {
      $('#Table1').DataTable({
        ...
        buttons: [
          {
            extend: 'excelHtml5',
            filename: 'state_wise_sales_return_ratio',
          },
          {
            extend: 'pdfHtml5',
            filename: 'state_wise_sales_return_ratio',
