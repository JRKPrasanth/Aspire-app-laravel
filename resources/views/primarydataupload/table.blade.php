...
@section('content')
  ...
  <h3 class="text-danger">Primary Data Upload</h3>


  <div class="card shadow-lg rounded-4 border-0">
    <div class="card-body">
      <form method="post" action="" id="primarydataupload" enctype="multipart/form-data">
        {{ @csrf_field() }}
        ...
        <div class="row g-3 mb-4 align-items-end">
          <!-- Download Template -->
          <div class="col-md-3">
            <a href="{{ asset('Uploads/PRIMARY_DB_TEMPLATE.csv') }}" class="btn btn-outline-primary w-100" download>
  ...
  <script>
    ...
    $(document).ready(function () {
      var table = $('#PrimaryTbl').DataTable({
        ...
        scrollY: "50vh",
        orderCellsTop: true,
        ajax: "{{ route('getprimarydatauploadData') }}",
        columns: [
          ...
          { data: 'state_consolidated', name: 'state_consolidated' },
          { data: 'hq', name: 'hq' },
          { data: 'stockist_dist_name', name: 'stockist_dist_name' },
          {
            data: 'primarydata_upload_id',
            name: 'actions',
            orderable: false,
            searchable: false,
    ...
    $('#myModal1').on('shown.bs.modal', function () {
      ...
      $(document).on('click', '.index', function () {
        ...
        if (form.parsley().isValid() && dup_chk == true) {
          ...
          $.ajax({
            url: "{{ URL::to('primarydatauploaddata') }}",
            type: "POST",
            data: formData,
            ...
            success: function (data) {
              ...
              form[0].reset();
              // Reload DataTable
              $('#PrimaryTbl').DataTable().ajax.reload();
            },
            error: function (xhr) {
              $('#myModal1').modal('hide');
    ...
    $(document).on('click', '.edit-btn', function () {
      ...
      const url = "{{ url('primarydatauploadedit') }}/" + id;
