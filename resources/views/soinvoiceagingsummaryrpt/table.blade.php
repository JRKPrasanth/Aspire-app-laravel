                ...
                <div class="card-body">
                  ...
                  <div class="table-responsive">
                    <table class="table table-bordered">
                      ...
                      <tbody>
                       <?php  //dd($result);
						  foreach($result as $key=>$value) {  if($value['customer_id']!=0) { ?>
						  
              <tr >
							
						  <td class="font-weight-medium">
                          <a class="btn-link supplier" data-value="{{$value['customer_id']}}" data-myvalue="0">{{ $value['customer_name'] }}</a> 
                          </td>
							 <td class="font-weight-medium">
                          <a class="btn-link supplier" data-value="{{$value['customer_id']}}" data-myvalue="1">
							  <?php 
                ...
                <?php 
                  if($value['advance_amount']== null){ echo "0.00";} else { echo number_format($value['advance_amount'],2);}?></a> 
							   ...
							   <td class="font-weight-medium">
                          <a class="btn-link supplier" data-value="{{$value['customer_id']}}" data-myvalue="2">
							 <?php 
								  if($value['lessthan16']== null){ echo "0.00";} else { echo number_format($value['lessthan16'],2);}?></a> 
							  </td>
							   <td class="font-weight-medium">
                          <a class="btn-link supplier" data-value="{{$value['customer_id']}}" data-myvalue="3">
                         <?php 
								  if($value['lessthan31']== null){ echo "0.00";} else { echo number_format($value['lessthan31'],2);}?></a> 
                          </td>
							  <td class="font-weight-medium">
                          <a class="btn-link supplier" data-value="{{$value['customer_id']}}" data-myvalue="4">
								<?php 
								  if($value['lessthan46']== null){ echo "0.00";} else { echo number_format($value['lessthan46'],2);}?></a> 
							  </td>
							  
							  <td class="font-weight-medium">
                          <a class="btn-link supplier" data-value="{{$value['customer_id']}}" data-myvalue="5">
								<?php 
								  if($value['above46']== null){ echo "0.00";} else { echo number_format($value['above46'],2);}?></a> 
							  </td>
                          <td class="font-weight-medium">
                          <a class="btn-link supplier" data-value="{{$value['customer_id']}}" data-myvalue="0">
								<?php 
...
// var from_date=$('.from_date').val(); 
    ...
    var urldate="{{ URL::to('soinvoiceagingsummaryrptdate')}}?to_date="+to_date;
	...
	$(document).on('click','.supplier', function(ev){
		...
		var url="{{ URL::to('pendingsalesreceipt')}}?customer_id="+customer+"&days="+val;
