...
@section('content')
<style type="text/css">
.bulk_tp_date{width: 120px;}
.bulk_divert_detail{width: 120px;}
.bulk_divert_reason{width: 120px;}
.bulk_from_area{width: 120px;}
.bulk_doctor_id{width: 120px;}
.bulk_focus_product{width: 290px;}
.bulk_timing{width: 120px;}
.bulk_activity_id{width: 120px;}
.bulk_visit_with{width: 250px;}
.bulk_outcome_id{width: 120px;}
.bulk_dcr_reminder_reason{width: 120px;}
.bulk_dcr_reminder_date{width: 120px;}
...
<div class="col-md-12">
    ...
    <div class="col-md-4">
        <div class="form-group row">
            <label for="active" class="form-control-label col-md-5">Tour Plan Date</label>
            <div class="col-md-6" style="pointer-events:none;">
                <input class="form-control input-sm tp_date datepicker" id="tp_date" name="tp_date" type="text" required value="{{ $tp_date }}" >
            </div>
        </div>
    </div>
    ...
    <div class="col-md-4">
        <div class="form-group row">
            <label for="active" class="form-control-label col-md-5">Tour Plan Deviation</label>
            <div class="col-md-6">
                <select name="tp_deviation" id="tp_deviation" class="form-control select2 tp_deviation">
        ...
        <div class="form-group row tp_div">
            <label for="active" class="form-control-label col-md-5">TP Deviation Reason</label>
            <div class="col-md-6">
                <textarea name="tp_deviation_reason" class="tp_deviation_reason form-control" class="tp_deviation_reason" >                    
                </textarea>
            </div>
        </div>
            ...
            <tbody class="line_data_body" >
                <?php if (count($linedata) >= 1) { ?>
                    @foreach($linedata as $key=>$value)
                        <tr class="clone rcopy1">
                            ...
                            <td>
                                <textarea id="bulk_divert_reason" name="bulk_divert_reason[]" class="form-control bulk_divert_reason" row="5"  >{!! $value->divert_reason !!}</textarea>
                            </td>
                            <td class="area_point">
                                <select name='bulk_from_area[]' class='form-control select2 bulk_from_area' id="bulk_from_area" required >
                                    {!! $value->from_area !!}
                                </select>
                            </td>
                            <td class="doctor_point">
                                <select name='bulk_doctor_id[]' rows='5' class='form-control select2 bulk_doctor_id' id="bulk_doctor_id" required >
                                    {!! $value->doctor_id !!}
                                </select>
                            </td>
                            <td>
                            ...
                            <td>
                                <input type="text" name="bulk_timing[]" class="form-control timepicker bulk_timing" value="{{$value->timing}}" required >
                            </td>
                            <td>
                                <select name='bulk_activity_id[]' class='form-control bulk_activity_id select2'  id="bulk_activity_id" required >
                                    {!! $value->activity_id !!}
                                </select>
                            </td>
                            <td>
                                <select name='bulk_visit_with[]' class='form-control bulk_visit_with select2' multiple id="bulk_visit_with" required >
                                    {!! $value->visit_with !!}
                                </select>
                            </td>
                            <td>
                                <select name='bulk_outcome_id[]' class='form-control bulk_outcome_id select2'  id="bulk_outcome_id" required >
                                    {!! $value->outcome_id !!}
                                </select>
                            </td>
                            <td>
                            ...
                            <td>
                                <textarea id="bulk_remarks" name="bulk_remarks[]" class="form-control bulk_remarks" row="5" required >{!! $value->remarks !!}</textarea>
                            </td>
                            <td>
                ...
                <?php } else { ?>
                    <tr class="clone rcopy1">
                        ...
                        <td>
                            <textarea id="bulk_divert_reason" name="bulk_divert_reason[]" class="form-control bulk_divert_reason" row="5"  ></textarea>
                            </td>
                        <td class="area_point">
                            <select name='bulk_from_area[]' class='form-control select2 bulk_from_area' id="bulk_from_area" required >
                                {!! $from_area !!}
                        ...
                        <td>
                            <input type="text" name="bulk_timing[]" class="form-control timepicker bulk_timing" value="" required >
                        </td>
                        <td>
                            <select name='bulk_activity_id[]' class='form-control bulk_activity_id select2'  id="bulk_activity_id" required >
                                {!! $activity_id !!}
                        ...
                        <td>
                            <textarea id="bulk_remarks" name="bulk_remarks[]" class="form-control bulk_remarks" row="5" required ></textarea>
                        </td>
                        <td>
                            <a href="#" class="bulk_pob_detail" title="Add Pob Details"> <i class="fa fa-plus"></i></a>
...
$(document).ready(function(){
    ...
    $(document).on('change','.tp_deviation',function(){
        var tpval = $('.tp_deviation option:selected').val();
        if(tpval == "Yes"){
            $('.tp_div').show();     
            $('.tp_deviation_reason').attr('required',true);
            $('.bulk_from_area,.bulk_doctor_id,.bulk_timing,.bulk_activity_id,.bulk_visit_with,.bulk_outcome_id,.bulk_remarks').removeAttr('required');
            $('.chandru').hide();
        }else{
            $('.chandru').show();     
            $('.tp_deviation_reason').attr('required',false);
            $('.bulk_from_area,.bulk_doctor_id,.bulk_timing,.bulk_activity_id,.bulk_visit_with,.bulk_outcome_id,.bulk_remarks').attr('required',true);
            $('.tp_div').hide();
        }
    });


    $('.area_point,.doctor_point').css('pointer-events','none');
        ...
        if(date != ''){
            ...
            $.get(url,function(data){
                ...
                if(dat == "INITIATED"){
                    ...
                    $('.bulk_divert_detail').css('pointer-events','none');
                    ...
                    $('.bulk_divert_detail').css('pointer-events','');
    ...
    $(document).on('change','.bulk_divert_detail',function(){
        ...
        var divert = $(this).val();
        if(divert == "Yes"){
            $(".bulk_from_area"+index).jCombo("{{ URL::to('jcomboformlogin?table=m_cities_t:city_id:city_name') }}&order_by=city_name asc",
                {selected_value:""});
            $('.area_point'+index).css('pointer-events','');
    ...
    $('.add_row1').click(function(){
        ...
        // $('.select2 ul li:nth-child(1)').css('display','none');
    ...
    $(document).on('click', '.remove', function ()
    {
        ...
        if(rowCount > 1 ){
            ...
            removeClass1('bulk_divert_reason');
            ...
            removeClass1('area_point');
            removeClass1('doctor_point');
        }else{
    ...
    $('input.timepicker').focus(function(){
        $(this).datetimepicker({
            weekStart: 1,
        todayBtn:  1,
        autoclose: true,
...
function changeclassfields1(){
    ...
    changeClassName('bulk_divert_reason');
    ...
    changeClassName('area_point');
    changeClassName('doctor_point');
}
...
$('.bulk_focus_product option,.bulk_visit_with option')
    .filter(function() {
