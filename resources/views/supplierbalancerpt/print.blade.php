...
class MYPDF extends TCPDF { 
 ...
 public function setLogo($company_logo,$sup_name,$st_date,$en_date){
        $this->cus_name = $sup_name;
 ...
 if($ccnopgp!=1)
{
 ...
 $this->SetXY(85,5);
    ...
    //$this->Cell(0,0, $this->sup_name ,0,1,'L');
...
$this->SetFont('times','','9');
$this->SetTextColor('0','0','0');
$this->cell(20,0,preg_replace("/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/i", "$1,", round($value->debit_amounts,2)),0,1,'R');

 $this->Setxy(153,$y);
$this->SetFont('times','','9');
$this->SetTextColor('0','0','0');
$this->MultiCell(20,2,preg_replace("/(\d+?)(?=(\d\d)+(\d)(?!\d))(\.\d+)?/i", "$1,", round($value->credit_amounts,2)),0,'R');
...
//$pdf-> setLogo($company_logo,$supplier_dat[0]->supplier_name,date('d-m-Y',strtotime($start_date)),date('d-m-Y',strtotime($end_date)));
...
$pdf->Line(80, 64, 130, 64, $style1);
    ...
    $pdf->SetTextColor('0','0','0');
    $pdf->Cell(0,0, $supplier_dat[0]->supplier_name ,0,1,'L');
    //dd($supplier_name);
    $y=$pdf->getY();
  $pdf->SetXY(22,$y);
    $pdf->SetFont('times','','10');
    $pdf->SetTextColor('0','0','0');
    $pdf->MultiCell(60,0, $supplier_dat[0]->supplier_site_name.", ".$supplier_dat[0]->address.", ".$supplier_dat[0]->city_name.", ".$supplier_dat[0]->state_name .", ".$supplier_dat[0]->country_name."- ".$supplier_dat[0]->pincode ,0,'L');
...
//     $pdf->Cell(0,0, "Phone-24780353,24780466" ,0,1,'L');
 ...
 $pdf->SetXY(120,$y);
    $pdf->SetFont('times','','9');
    $pdf->SetTextColor('0','0','0');
    $pdf->Cell(0,0, "Regd Off: 13, Perumal Koil Street" ,0,1,'L');
 ...
 $pdf->SetXY(120,$y);
    ...
    $pdf->SetFont('times','','11');
    $pdf->SetTextColor('0','0','0');
    $pdf->Cell(0,0, "Sub: Account Statement" ,0,1,'L'); 
...
//ob_end_clean();
	  ...
	  ob_end_clean();
	  if(isset($supplier_dat[0]->supplier_name)){
	   $pdf->Output($supplier_dat[0]->supplier_name.'.pdf','I');    
	  }else{
 $pdf->Output('No_supplier_name.pdf','I'); 
	  }
	exit;
