...
@section('content')
<h3 class="text-danger">EL Encashment Approve</h3>
@include('layouts.breadcrumb')

<button type='button'  class='btn btn-success mt-4 request'><i class="bi bi-check2-circle"></i> EL Encashment Approve</button>
...
$(document).ready(function () {
    ...
    $(document).on('click', '.request', function () {
        ...
        if (selectedIds.length > 0) {
            var url = "{{ URL('approveelencashment') }}/?row_id=" + selectedIds.join(',');
            $.get(url, function (data) {
                if (data == 1) {
                    showCustomAlert('El Encashment Approved Successfully','success');
                    table.ajax.reload(); 
                }
            });
        } else {
