...
@section('content')
  <h3 class="text-danger">Team Target Vs Achivement For the Month</h3>
  ...
  <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
    <form action="{{ url('salesandtargetteammonth') }}" method="get" id="searchForm">
      ...
      <table id="Table1" class="table table-bordered table-striped table-hover w-100">
        <thead>
          <tr>
            ...
            <?php endif; ?>
            <th colspan="6" class="align text-white bg-danger text-center">TEAM WISE SALES AND TARGET FOR ONLY -
              {{ $mon_yr }}</th>
          </tr>
          <tr>
            <th colspan="2" class="align text-white bg-secondary text-center"></th>
            <th colspan="1" class="align text-white bg-secondary text-center">Sales</th>
          ...
          <tr>
            ...
            <th class="align bg-warning">Comman Fieldforce Name</th>
            <th class="align bg-warning"><span class="text-warning" style="font-size:6px;">Sale </span>{{ $mon_yr }}</th>
            <th class="align bg-warning"><span class="text-warning" style="font-size:6px;">Target </span>{{ $mon_yr }}
            </th>
            <th class="align bg-warning"><span class="text-warning" style="font-size:6px;">Target </span>{{ $nxt_target }}
            </th>
  ...
  $grandTotalSale = 0;
  $grandTotalTarget = 0;
  $grandTotalTargetnxt = 0;
  foreach ($all_ind_sal as $value) {
    $grandTotalSale += $value->sale;
    $grandTotalTarget += $value->target;
    $grandTotalTargetnxt += $value->nxt_target;
          ?>
          <tr>
            <td>{{ $value->name}}</td>
            <td class="sticky-col">{{ $value->field_name}}</td>
            ...
            <td class="rupee-value" data-original="{{ $value->target}}">{{ $value->nxt_target}}</td>
      ...
      echo '0';
        ...
        <tfoot class="table-danger">
          ...
          <tr class="sticky-foot fw-bold">
            <td>Grand Total</td>
            <td></td>
            <td class="rupee-value" data-original="{{ $grandTotalSale }}">{{ $grandTotalSale }}</td>
            <td class="rupee-value" data-original="{{ $grandTotalTarget }}">{{ $grandTotalTarget }}</td>
            <td class="rupee-value" data-original="{{ $grandTotalTarget }}">{{ $grandTotalTargetnxt }}</td>
            <td <?php echo getBackgroundColor($grandTotalSale, $grandTotalTarget); ?>>
              <?php
  if ($grandTotalTarget != 0) {
    echo round(($grandTotalSale / $grandTotalTarget) * 100, 0) . '%';
  } else {
    echo '0';
  }
                      ?>
...
@push('scripts')
  ...
  <script>
    ...
    document.getElementById('searchForm').addEventListener('submit', function (event) {
      ...
      if (zone === '' && region === '' && startDate === '' && manager === '' && state === '') {
