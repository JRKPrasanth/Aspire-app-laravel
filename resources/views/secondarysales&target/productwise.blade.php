...
@section('content')
  ...
  <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
    <form action="{{ url('salesandtargetproduct') }}" method="get" id="searchForm">
      <div class="row g-3 align-items-end">
        ...
        <div class="col-md-3">
          ...
          <select name="state" id="state" class="form-select select2 state text-center">
            <option value="">-- please select --</option>
        ...
        <!-- Search Button -->
        <div class="col-5 text-center mt-">
      ...
      <table id="Table1" class="table table-bordered table-striped table-hover w-100">
        <thead>
          <tr>
            ...
            <th class="align text-white bg-danger text-center"><?php  echo request('division'); ?></th>
            <?php endif; ?>
            <?php if (request('state') != ''): ?>
            <th class="align text-white bg-danger text-center"><?php  echo request('state'); ?></th>
            <?php endif; ?>
            <?php if (request('area') != ''): ?>
            <th class="align" style="background:#9b900d">{{ request('area') }}</th>
            <?php endif; ?>
            <th colspan="9" class="align text-white bg-danger text-center">PRODUCT WISE TARGET ACHIVEMENT UPTO -
              {{ $mon_yr }}</th>
          ...
          <tr>
            ...
            <th class="align bg-warning">Division</th>
            <th class="align bg-warning">Product Name</th>
            <th class="align bg-warning"><span class="text-warning" style="font-size:6px;">Purchase
              </span>{{ $pre_fy_year }}</th>
            <th class="align bg-warning"><span class="text-warning" style="font-size:6px;">Purchase
              </span>{{ $cur_fy_year}}</th>
            <th class="align bg-warning">Increase/Decrease</th>
            <th class="align bg-warning">Growth</th>
            <th class="align bg-warning"><span class="text-warning" style="font-size:6px;">Target </span>{{ $cur_fy_year}}
            </th>
            <th class="align bg-warning">Short Fall In Target</th>
  ...
  ?>
          <tr>
            ...
            <td class="rupee-value"
              data-original="<?php  echo $data['primary_dis_cur_fy'] - $data['primary_dis_pre_fy']; ?>">
              <?php  echo $data['primary_dis_cur_fy'] - $data['primary_dis_pre_fy']; ?></td>
            <?php  if ($data['primary_dis_pre_fy'] != 0 && $data['primary_dis_cur_fy'] != 0) { ?>
            <td><?php    echo round(($data['primary_dis_cur_fy'] / $data['primary_dis_pre_fy'] - 1) * 100, 0); ?>%</td>
            <?php  } else { ?>
            <td>0</td> <?php  } ?>
            <td class="rupee-value" data-original="<?php  echo $data['target_cur_fy']; ?>">
              <?php  echo $data['target_cur_fy']; ?></td>
            <td class="rupee-value" data-original="<?php  echo $data['primary_dis_cur_fy'] - $data['target_cur_fy']; ?>">
              <?php  echo $data['primary_dis_cur_fy'] - $data['target_cur_fy']; ?></td>
            <?php  if ($data['target_cur_fy'] != 0) { ?>
            <td
              style="border-right: 1px solid #000; text-align: center; <?php    echo getBackgroundColor($data['primary_dis_cur_fy'], $data['target_cur_fy']); ?>">
        ...
        <tfoot class="table-danger">
          <tr class="sticky-foot fw-bold">
            ...
            <td class="rupee-value"
              data-original="<?php echo $divisionTotals['totalPrimaryDis_cur_fy'] - $divisionTotals['totalPrimaryDis_pre_fy']; ?>">
              <?php echo $divisionTotals['totalPrimaryDis_cur_fy'] - $divisionTotals['totalPrimaryDis_pre_fy']; ?></td>
            <?php  if ($divisionTotals['totalPrimaryDis_pre_fy'] != 0 && $divisionTotals['totalPrimaryDis_cur_fy'] != 0) { ?>
            <td>
              <?php  echo round(($divisionTotals['totalPrimaryDis_cur_fy'] / $divisionTotals['totalPrimaryDis_pre_fy'] - 1) * 100, 0); ?>%
            </td><?php } else { ?>
            <td>0</td> <?php } ?>
            ...
            <td class="rupee-value"
              data-original="<?php echo $divisionTotals['totalPrimaryDis_cur_fy'] - $divisionTotals['totaltarget_cur_fy']; ?>">
              <?php echo $divisionTotals['totalPrimaryDis_cur_fy'] - $divisionTotals['totaltarget_cur_fy']; ?></td>
            <?php  if ($divisionTotals['totaltarget_cur_fy'] != 0) { ?>
            <td
              style="border-right: 1px solid #000; text-align: center; <?php  echo getBackgroundColor($divisionTotals['totalPrimaryDis_cur_fy'], $divisionTotals['totaltarget_cur_fy']); ?>">
    ...
    $(document).ready(function () {
      $('#Table1').DataTable({
        ...
        buttons: [
          {
            extend: 'excelHtml5',
            filename: 'Product Wise TARGET ACHIVEMENT',
            exportOptions: {
              format: {
