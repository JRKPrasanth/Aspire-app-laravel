...
<h2 class="heads">LEAVE APPLICATION</h2>
    ...
    <div class="card-body card-block">
        ...
        <div class="row">
                ...
                <div class="form-group col-md-4">
                    <label for="start_date" class="form-control-label col-md-5"><span class="red">*</span>Employee</label>
                ...
                <div class="form-group col-md-4">
                    <label for="start_date" class="form-control-label col-md-3"><span class="red">*</span>Leave Type</label>
                    <div class="col-md-2" >LB:<span class="balance">0</span></div>
                    <div class="col-md-6" >
		...
		$("#grid1").jqGrid(
            {
               datatype: "local",
                colModel: [
                    ...
                    { name: "employee_name", label: "Employee Name", width: 250},
                    ...
                    { name: "end_date", label: "End Date", width: 250 ,editable:true, formatter: 'date',editrules:{date:true}, formatoptions: {  newformat: date_format}},
                     { name: "leave_combo", label: "Leave Combo", width: 250 ,editable:true, formatter: 'date',editrules:{date:true}, formatoptions: {  newformat: date_format}},
                ...
                //    { name: "approval_reason", label: "Approve Reason", width: 250},
                  //  { name: "approvel_comments", label: "Approve Comments", width: 250},
                    ...
                    { name: "leave_type", label: "Leave Type", width: 250,hidden: true},
	 ...
	 $(document).on('click',".exportpdf",function() {
        ...
        $(document).on('click',".exportexcel",function() 
        {
            $("#grid1").jqGrid("exportToExcel",{
                ...
                includeGroupHeader : true,
                includeFooter: true,
                fileName : "Leave Application.xlsx"
 ...
 $(document).on('click',".jcr_leave_type",function() {
  ...
  {selected_value:""});
   ...
   /***** Clear Function Start  ****/
        ...
        {
            $('#edit_id').val('');
            $('#leave_type').val('').select2();
            $('#leave_mode').val('').select2();
            $('#start_date').val('');
            $('#end_date').val('');
            $('#start_date1').val('');
            $('#end_date1').val('');
            ...
            $('#leave_mode').select('');
            $('#alloted_days').val('');
            $('#approvel_comments').val('');
            $('#approval_reason').val('');
            $("#leave").parsley().destroy();
            location.reload();
        });
               /***** Clear Function End  ****/ 
                    ...
                    {	
                       ...
                       var data	= $('#leave').serialize();
                        $.post(url,data,function(data1)
                        {
                            if(data1 == 1)
                            {
                                notyMsg('success','Leave Application Saved Successfully');
                          location.reload();
                                $('.reset').trigger('click');
                            }
                            else
                            {
                                notyMsg('success','Leave  Application Updated Successfully'); 
                                location.reload();
                                $('.reset').trigger('click');
                            }
                        });
