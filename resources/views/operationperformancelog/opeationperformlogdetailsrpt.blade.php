...
<h3 class="text-danger">Employee Performance Report</h3>
...
<div class="container mt-4">
    <div class="row g-3 mb-3">
        ...
        <div class="col-12 col-md-3">
            <a href="overalloperationperformancereport" class="btn btn-outline-primary w-100">Overall Employee Report</a>
        </div>
        <div class="col-12 col-md-3">
            <a href="operationperformancelogreport" class="btn btn-warning w-100">Employee Performance Report</a>
        </div>
        <div class="col-12 col-md-3">
...
<div class="card shadow-lg rounded-4 border-0 p-4">
    <form action="{{ url('operationperformancelogreport') }}" method="get" id="searchForm">
        <div class="row g-4">

            <!-- Supplier Name -->
            <div class="col-md-4">
                <label for="emp_name" class="form-label fw-semibold">Employee Name</label>
                <select name="emp_name" id="emp_name" class="form-select emp_name select2" required>
                    {!! $employee_name !!}
                </select>
            </div>
...
<div class="card shadow-lg rounded-4 border-0 p-4">
	 <div class="col-md-12">
		 <h5 class="text-danger text-center">Employee Work Hours Report</h5> 
	</div>
        ...
        <thead>
              <?php if (empty($prim_summary)) { ?>
                ...
                <tbody>
                    ...
                    <?php $categories = array_unique(array_column($prim_summary, 'type')); ?>
                    <?php $grandTotalRunningHrs = $grandTotalIdleHrs = 0; ?>
                    <?php $columnTotals = array_fill(0, count($displayedMonths) * 2, 0); ?>

                    <?php foreach ($categories as $category) : ?>
                        <tr>
                            <td style="text-decoration: underline;cursor: pointer;" class="sticky-col Typewise"><?= $category ?></td>
                            <?php $totalRunningHrs = $totalIdleHrs = 0; ?>
                            <?php $colIndex = 0; ?>
                            <?php foreach ($displayedMonths as $month) : ?>
                                <?php $foundData = false; ?>
                                <?php foreach ($prim_summary as $value) : ?>
                                    <?php if ($value->type == $category && $value->yr_month == $month) : ?>
                                        ...
                                        <?php $totalIdleHrs += timeToSeconds('200:00:00') - timeToSeconds($value->wrk_hrs); ?>
                                        <?php /* Update column totals */ ?>
                                        <?php $columnTotals[$colIndex] += timeToSeconds($value->wrk_hrs); ?>
                                        <?php $columnTotals[$colIndex + 1] += timeToSeconds('200:00:00') - timeToSeconds($value->wrk_hrs); ?>
                                        <?php $foundData = true; ?>
                                    <?php endif; ?>
                                <?php endforeach; ?>
                                <?php if (!$foundData) : ?>
                ...
                <tbody>
                    ...
                    <?php foreach ($categories as $category) { ?>
                        ...
                        <?php foreach ($typesWithCategoryData as $type) { ?>
                            <tr>
                                <td style="text-decoration: underline;cursor: pointer;" class="sticky-col Prowise"><?= $category ?></td>
              ...
              <?php if (empty($machine_summary)) { ?>
                ...
                <tbody>
                    ...
                    <?php foreach ($categories as $category) { ?>
                        ...
                        <?php foreach ($typesWithCategoryData as $type) { ?>
                            <tr>
                                <td style="text-decoration: underline;cursor: pointer;" class="sticky-col Machwise"><?= $category ?></td>
              ...
              <?php if (empty($type_summary)) { ?>
                ...
                <tbody>
                    ...
                    <?php foreach ($categories as $category) { ?>
                        ...
                        <?php foreach ($typesWithCategoryData as $type) { ?>
                            <tr>
                                <td style="text-decoration: underline;cursor: pointer;" class="sticky-col Categorywise"><?= $category ?></td>
              ...
              <?php if (empty($name_summary)) { ?>
                ...
                <tbody>
                    ...
                    <?php foreach ($categories as $category) { ?>
                        ...
                        <?php foreach ($typesWithCategoryData as $type) { ?>
                            <tr>
                                <td style="text-decoration: underline;cursor: pointer;" class="sticky-col Productwise"><?= $category ?></td>
...
<!-- Employee Product Name Wise Work Report Modal -->
<div class="modal fade" id="EmpprodctModal" tabindex="-1">
  <div class="modal-dialog modal-xl modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header bg-secondary">
        <h5 class="modal-title text-white">Employee Product Name Wise Work Report</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="row mb-3">
          ...
          <div class="col-md-6">
            <h6>Product Name: <strong><span id="product_name"></span></strong></h6>
          </div>
          <div class="col-md-2">
            <h6>Type: <strong><span id="product_type"></span></strong></h6>
          </div>
        </div>
        <div style="height: 450px; overflow-y: auto;">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#Table1').DataTable({
                ...
                buttons: [
                    ...
                    { extend: 'excelHtml5', title: "Employee Work Hours Report", exportOptions: { columns: ':visible' } }
                ]
        });
    ...
    // popup scripts
       ...
       // modal for type wrk hrs rpt
       
        $(document).ready(function() {
         $('.Typewise').click(function() {
        // Show the modal
        ...
        var type = row.find('.sticky-col.Typewise').text().trim();
        ...
        if (type) {
            ...
            var url = "{{ route('employetypepopup') }}" + "?" + params;

            // AJAX request to fetch data
            $.get(url, function(data) {
                // Update HTML content with received data
                $('#employeetypetable').html(data);
            });
        } else {
    ...
    $('#EmptypeModal').on('hidden.bs.modal', function () {
        $('#employeetypetable').html('');  
        $('#employee_pname').text('');  
    });
 });


       // modal for total wrk hrs rpt
       
        $(document).ready(function() {
         $('.totalwrkpop').click(function() {
        // Show the modal
        $('#OverallwrkModal').modal('show');
        ...
        if (type) {
            ...
            // Build the full URL for the AJAX request
            var url = "{{ route('totalwrkemp') }}" + "?" + params;

            // AJAX request to fetch data
            $.get(url, function(data) {
                // Update HTML content with received data
                $('#employeoveralltable').html(data);
            });
        } else {
            console.log("Type not found.");
        }
    ...
    // Close modal button to close the sidebar
    $('#closeButton').click(function() {
        $('#OverallwrkModal').modal('hide');
    });
    
    // Clear modal content when it is hidden
    $('#OverallwrkModal').on('hidden.bs.modal', function () {
        $('#employeoveralltable').html('');  
     //  $('#employee_pname').text('');  
    });
 });
...
if (prodct) {
    ...
    // Build the full URL for the AJAX request
    var url = "{{ route('employeproductpopup') }}" + "?" + params;
...
$(document).ready(function() {
   ...
   if (machine) {
       ...
       // Build the full URL for the AJAX request
       var url = "{{ route('employemachinepopup') }}" + "?" + params;
   ...
   if (category) {
       ...
       // Build the full URL for the AJAX request
       var url = "{{ route('employecatepopup') }}" + "?" + params;
...
$(document).ready(function() {
   ...
   if (product) {
       ...
       // Build the full URL for the AJAX request
       var url = "{{ route('employepropopup') }}" + "?" + params;
