...
@section('content')
    ...
    <div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
        <form action="{{ url('primarycostsaleratioregion') }}" method="get" id="searchForm">
                    ...
                    <table id="Table1" class="table table-bordered table-striped table-hover w-100">
                        <thead>
                            <tr>
                                ...
                                <?php endif; ?>
                                <th colspan="3" class="align text-white bg-danger text-center">{{ $cur_fy_year }} upto
                                    {{ $mon_yr}}</th>
                                <th colspan="3" class="align text-white bg-danger text-center">ALL INDIA</th>
                            </tr>
                            <tr>
                                <th class="align align text-white bg-secondary text-center"> Region</th>
                                <th class="align align text-white bg-secondary text-center"> Sales</th>
                                <th class="align align text-white bg-secondary text-center"> Salary\Exp.\Incen</th>
                                <th class="align align text-white bg-secondary text-center"> Manpower</th>
                                <th class="align align text-white bg-secondary text-center"> % of Cost to Sales Ratio</th>
                                <th class="align align text-white bg-secondary text-center"> Per Person Productivity</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
    ...
    foreach ($month_wise as $value) {
        ...
        $totalMan += $value->man;
                ?>
                            <tr>
                                ...
                                <?php    if ($value->sale != '0') { ?>
                                <td>
                                    {{ number_format($value->expence / $value->sale * 100, 0) }}%
                                </td>
                                <?php    } else { ?>
                                <td>0</td>
                                <?php    } ?>
        ...
        if (
            isset($value->man) && is_numeric($value->man) && $value->man !== '0' &&
        ...
        echo str_replace(',', '', $result);
                        ...
                        <tfoot>
                            <tr class="fw-bold table-danger">
                                ...
                                <td>{{ $totalMan }}</td>
                                <td>
                                    <?php if ($totalExpence && $totalSale != 0) {
        echo number_format($totalExpence / $totalSale * 100, 0);
    } else {
        echo '0';
    } ?>%
                                </td>
    ...
    if ($totalSale && $totalMan != '0') {
        ...
        $value_lak = 0; // Define default value if $value->man is 0
                    ...
                    ?>
                                <td class="rupee-value" data-original="<?php echo $value_lak; ?>">
                                    <?php if ($totalMan && $totalSale != 0) {
        echo number_format($totalSale / $totalMan, 0);
    } else {
        echo '0';
    } ?>
                                </td>
        ...
        $totalMan += $value->man;
                ?>
                            <tr>
                                ...
                                <td>{{ $value->man }}</td>
                                <?php    if ($value->sale != '0') {  ?>
                                <td>{{ number_format($value->expence / $value->sale * 100, 0) }}%</td> <?php    } else { ?>
                                <td>0</td> <?php    } ?>
                                <?php
        // Calculate $result before using it
        // Calculate $result before using it
        ...
        echo str_replace(',', '', $result);
                        ...
                        <tfoot>
                            <tr class="fw-bold table-danger">
                                ...
                                <td class="rupee-value" data-original="{{ $totalExpence }}">{{ $totalExpence }}</td>
                                <td>{{ $totalMan }}</td>
                                <td><?php  if ($totalExpence && $totalSale != 0) {
        echo number_format($totalExpence / $totalSale * 100, 0);
    } else {
        echo '0';
    } ?>%
    ...
    <div class='row'>
        ...
        <div class='col-md-6 card shadow-lg rounded-4 border-0 p-4'>
