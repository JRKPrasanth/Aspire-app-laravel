...
@section('content')
<h3 class="text-danger">Person Wise Manpower</h3>
...
<div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
  <form action="{{ url('primarycostratiomanpower') }}" method="get" id="searchForm">
    ...
    <div class="table-responsive" style="overflow-x: auto;">
        ...
        <thead>
            <tr>
                ...
                <th colspan="13" class="align text-white bg-danger text-center" >ALL INDIA -  DISTRIBUTOR WISE SALE VALUE - upto {{ $mon_yr}} </th>
            </tr>
        ...
        <!-- Display the grand total row -->
            <tr class="sticky-foot fw-bold table-danger" >
                ...
                <?php foreach ($displayedMonths as $month) : ?>
                    <td class="rupee-value" data-original="<?php echo $grandTotal[$month]; ?>" ><?php echo $grandTotal[$month]; ?></td>
                <?php endforeach; ?>
            </tr>
  </tfoot>
        </table>
        ...
        $(document).ready(function() {
            $('#Table1').DataTable({
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        filename: 'cost_to_sale_PERSON_WISE_MANPOWER',
                    },
                    {
                        extend: 'pdfHtml5',
                        filename: 'cost_to_sale_PERSON_WISE_MANPOWER',
