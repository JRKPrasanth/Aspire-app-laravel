...
<div class="card shadow-lg rounded-4 border-0 p-4 mb-4">
  <form action="{{ url('primarycostsaleratiostate') }}" method="get" id="searchForm">
   ...
   <tbody>
    ...
    ?>
        <tr>
            ...
            <td >{{ $value->man }}</td>
               <?php if($value->sale !='0'){  ?><td >{{ number_format($value->expence / $value->sale * 100, 0) }}%</td> <?php }else{ ?><td >0</td> <?php } ?> 
