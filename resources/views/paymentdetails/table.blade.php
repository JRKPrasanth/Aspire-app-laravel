...
@section('content')
  ...
  <div class="row mb-3 mt-2">
    <div class="col-md-12 d-flex flex-wrap align-items-center gap-2">
      ...
      <!-- Direct Payment -->
      <button type="button" id="create" value="1" class="btn btn-primary px-4 d-flex align-items-center">
        <i class="fa fa-money-bill-wave me-2"></i> Direct Payment
      </button>

      <!-- Hidden field -->
      <input type="hidden" class="payment"><a href='' style="dispaly:none;" class='download_file' download></a>

      <!-- Same Bank -->
      <a href="" class="btn btn-success px-4 d-flex align-items-center" id="paymentsamedwnld" download>
        <i class="fa fa-university me-2"></i> Same Bank Details
      </a>

      <!-- Other Bank -->
      <a href="" class="btn btn-info px-4 d-flex align-items-center" id="paymentothrbankdwnld" download>
        <i class="fa fa-exchange-alt me-2"></i> Other Bank Details
      </a>

      <!-- BOB Bank -->
      <a href="" class="btn btn-warning px-4 d-flex align-items-center" id="paymentbobdwnld" download>
        <i class="fa fa-building-columns me-2"></i> BOB Bank Details
      </a>

      <!-- Credit Card -->
      <a href="" class="btn btn-danger px-4 d-flex align-items-center" id="paymenticicicrtcarddwnld" download>
        <i class="fa fa-credit-card me-2"></i> Credit Card Details
      </a>

      <!-- Payment Cancellation -->
      <button type="button" id="chequecancellation" class="btn btn-secondary px-4 d-flex align-items-center mt-4 me-2">
        <i class="fa fa-ban me-2"></i> Payment Cancellation
      </button>

      <!-- Payment Advice -->
      <button type="button" id="paymentadvice" class="btn btn-teal text-white px-4 d-flex align-items-center mt-4 me-2"
        style="background-color:#20c997;">
        <i class="fa fa-file-invoice-dollar me-2"></i> Payment Advice
      </button>

      <!-- Cheque -->
      <button type="button" id="cheque" class="btn btn-indigo text-white px-4 d-flex align-items-center mt-4 me-2"
        style="background-color:#6610f2;">
        <i class="fa fa-file-signature me-2"></i> Cheque
      </button>

      <!-- Voucher -->
      <button type="button" id="voucher" class="btn btn-rose text-white px-4 d-flex align-items-center mt-4 me-2"
...
@push('scripts')
  ...
  <script>
    ...
    $("#create").click(function () {
      window.location.replace('directpaymentcreate');
    });
    ...
    // credit card details
    $("#paymenticicicrtcarddwnld").on('click', function (e) {
      e.preventDefault();
      ...
      if (selectedIds.length > 0) {
        var url = "{{ url('paymentsamedownloaddata') }}?payment_id=" + selectedIds.join(',') + "&type=icicicrdcard";

        $.get(url, function (data) {
          if (data.download_ling) {
            $(".download_file").attr("href", data.download_ling)[0].click();
          } else {
            showCustomAlert("No download link available for selected payments", "info");
          }
        });
      } else {
        showCustomAlert("Please choose at least one payment to download", "info");
      }
    });
    ...
    $("#paymentbobdwnld").on('click', function (e) {
      e.preventDefault();
      ...
      if (selectedIds.length > 0) {
        var url = "{{ url('paymentsamedownloaddata') }}?payment_id=" + selectedIds.join(',') + "&type=bob";

        $.get(url, function (data) {
          if (data.download_ling) {
            $(".download_file").attr("href", data.download_ling)[0].click();
          } else {
            notyMsg("info", "No download link available for selected payments");
          }
        });
      } else {
        showCustomAlert("Please choose at least one payment to download", "info");
    ...
    $("#paymentothrbankdwnld").on('click', function (e) {
      e.preventDefault(); // Prevent default behavior

      var selectedIds = [];

      // Collect all checked rows (checkboxes inside your table body)
      $('#AccTbl tbody input.row-checkbox:checked').each(function () {
        selectedIds.push($(this).val());
      });

      // If no rows selected, show alert and stop
      if (selectedIds.length === 0) {
        showCustomAlert("Please select at least one payment record", "info");
        return false;
      }

      // Proceed with download
      var url = "{{ url('paymentsamedownloaddata') }}?payment_id=" + selectedIds.join(",") + "&type=other";

      $.get(url, function (data) {
        if (data.download_ling) {
          $(".download_file").attr("href", data.download_ling);
          $(".download_file")[0].click();
        } else {
          showCustomAlert("File not found or download link missing", "error");
        }
      }).fail(function () {
        showCustomAlert("Error fetching file. Please try again.", "error");
      });
    });

    // same bank details	
    $("#paymentsamedwnld").on('click', function (e) {
      e.preventDefault(); // Prevent default behavior

      var selectedIds = [];

      // Collect all checked checkboxes from the table
      $('#AccTbl tbody input.row-checkbox:checked').each(function () {
        selectedIds.push($(this).val());
      });
      ...
      var url = "{{ url('paymentsamedownloaddata') }}?payment_id=" + selectedIds.join(",") + "&type=same";

      $.get(url, function (data) {
        if (data.download_ling) {
          $(".download_file").attr("href", data.download_ling);
          $(".download_file")[0].click(); // trigger download
        } else {
          showCustomAlert("File not found or download link missing", "error");
        }
      }).fail(function () {
