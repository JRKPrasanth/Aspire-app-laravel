...
class PurchasereturnController extends Controller
{
 ...
 /* Karthigaa purpose for Display Purchase Return Data in JQgrid function */
    public function purchasereturnData(){
        $wh='';
             if($_GET['status']!=''){
		  $wh=" and  p_return_header_t.p_return_status='".$_GET['status']."'";
	  }
        if($_GET['_search']=='true'){
			$search_tables=array('m_supplier_t','p_po_invoice_hdr_t');
                        $wh.=$this->jqgridsearch('p_return_header_t',$_GET['filters'],$search_tables);
        }
        ...
        $wh.='and  p_return_header_t.company_id='.$compy;
        }else{
            $wh.='and  p_return_header_t.company_id='.$compy.' and p_return_header_t.location_id='.$loc;
        }
        ...
        $sord = $_GET['sord'];
        if(!$sidx) $sidx =1;
        $result = \DB::select("SELECT COUNT(return_header_id) AS count FROM p_return_header_t left join m_supplier_t on(
                        m_supplier_t.supplier_id=p_return_header_t.`supplier_id`) left join p_po_invoice_hdr_t on(
                        p_po_invoice_hdr_t.po_invoice_id=p_return_header_t.`invoice_number`) where 1=1 $wh");
        ...
        if($start <0) $start = 0;
                $SQL = "SELECT
                        ...
                        m_supplier_t.supplier_id=p_return_header_t.`supplier_id`) left join p_grn_hdr_t on p_grn_hdr_t.grn_id=p_return_header_t.grn_number  where 1=1 $wh ORDER BY $sidx $sord LIMIT $start , $limit";
                 $download_SQL = "SELECT
                        p_return_header_t.return_header_id,
                        p_return_header_t.return_date,
                        p_return_header_t.return_invoice_number,
                        ...
                        p_po_invoice_hdr_t.po_invoice_id=p_return_header_t.`invoice_number`)
                        left join m_supplier_t on(
                        m_supplier_t.supplier_id=p_return_header_t.`supplier_id`) left join p_grn_hdr_t on p_grn_hdr_t.grn_id=p_return_header_t.grn_number  where 1=1 $wh ORDER BY $sidx $sord";
$result1 = \DB::select( $download_SQL );
 ...
 /* Karthigaa purpose for Display Invoice Data in JQgrid function */
       public function invoiceData(){
        ...
        $wh='';
        if($_GET['_search']=='true'){
	$search_tables=array('p_qc_header_t','p_qc_lines_t','p_quality_spec_trx_lines_t','m_supplier_t','m_subcontract_supplier_t','p_po_hdr_t','p_po_invoice_hdr_t');
        $wh=$this->jqgridsearch('p_grn_hdr_t',$_GET['filters'],$search_tables);
        ...
        $sord = $_GET['sord'];
        if(!$sidx) $sidx =1;
        $result = \DB::select("SELECT COUNT(grn_id) AS count FROM `p_grn_hdr_t`
        ...
        if($start <0) $start = 0;
                $SQL = "SELECT
                        ...
                        p_qc_lines_t.reject_qty,
                        p_quality_spec_trx_lines_t.quality_status
                        FROM
                            `p_grn_hdr_t`
                            LEFT JOIN p_qc_header_t ON(
                                p_qc_header_t.grn_number = p_grn_hdr_t.grn_id
                            )
                        LEFT JOIN p_qc_lines_t ON(
                                p_qc_lines_t.qc_header_id = p_qc_header_t.qc_header_id
                            )
                        LEFT JOIN p_quality_spec_trx_lines_t ON(
                                p_quality_spec_trx_lines_t.reference_source_line_id = p_qc_lines_t.qc_line_id
                            )
                        LEFT JOIN p_po_hdr_t ON(
                                p_po_hdr_t.po_hdr_id = p_qc_header_t.`po_number`
                            )
                        LEFT JOIN m_subcontract_supplier_t ON(
                                m_subcontract_supplier_t.subcontract_supplier_id = p_qc_header_t.`subcontract_supplier_id`
                            )
                        LEFT JOIN m_supplier_t ON(
                                m_supplier_t.supplier_id = p_qc_header_t.`supplier_id`
                            )

                        LEFT JOIN p_po_invoice_hdr_t ON
                            (
                                p_po_invoice_hdr_t.grn_number = p_grn_hdr_t.grn_id
                            )
                        WHERE
                            1 = 1 AND p_grn_hdr_t.invoice_created = 'Yes' AND p_qc_header_t.return_status = 0 AND p_qc_lines_t.reject_qty != 0 AND p_po_invoice_hdr_t.po_invoice_status = 'APPROVED' $wh
                        GROUP BY
                            p_qc_header_t.grn_number  ORDER BY $sidx $sord LIMIT $start , $limit";
        $result = \DB::select( $SQL );
        $responce->rows[]='';
        $responce->rows=$result;
        $responce->page = $page;
...
/*End Purpose For Edit Function*/
    ...
    if($_POST['p_return_status']=="APPROVED"){
                  ...
                  // $jid = DB::getPdo()->lastInsertId();
                   ...
                   $invoiceid=DB::table('p_return_lines_t')->where('return_header_id',$id)->get();
echo"<pre>";
