...
class InvestmenttypeController extends Controller
{
	...
	public function hrmsallowancesettings()
    {
	$this->data['pageMethod']=\Request::route()->getName();	
    ...
    public function hrmsallowancesettingsindex($id=null)
    {
		if($id == 0)
		{
			...
			$this->data['line_data'] = array();
                         $this->data['allowance_id']                   = $this->jcombojoinselect('m_allowance_tbl','allowance_id','allowance_name','a_lookuplines_t','lookuplines_id','employee_type','lookup_code','',"");
		...
		else
		{
			...
			$this->data['line_data'] = $lines_result;	
                        foreach($lines_result as $key=>$value){
                           $lines_result[$key]->allowance_id=$this->jcombojoinselect('m_allowance_tbl','allowance_id','allowance_name','a_lookuplines_t','lookuplines_id','employee_type','lookup_code',$value->allowance_id,"");
   ...
   public function destroy(Request $request)
    {
       ...
       /**  Delete Start **/
   public function hrmsallowancesettingsdelete(Request $request)
    {
...
/**Jqgrudload data Start **/
    public function hrmsallowancesettingsgrid(investmenttype $investmenttype)
    {
		...
		$search_tables=[""];
		  if($_GET['_search']=='true'){
         $wh=$this->jqgridsearch("f_hr_account_allowance_setting_t",$_GET['filters'],$search_tables);
       } 
	if(!$sidx) $sidx =1;
        
           $result = \DB::select("SELECT COUNT(account_allowance_setting_id) AS count FROM f_hr_account_allowance_setting_t where 1=1 $wh");
