...
class SecondarysalesandtargetController extends Controller
{

		public function __construct(){
        
    $this->pageModule="salesandtargetmonthindex";	
    $this->data=array();
...
// login viewers track purpose End
 ...
 return view('secondarysales&target.index', $this->data);
        ...
        if ($depart == "14" && $emp_id != '121') {
            ...
            if ($emp_reg == '41') {
                ...
                if (!empty($em_region)) {
                    ...
                    $wh = " AND sd_secondarydataupload_t.region IN ($emp_region)";
                } else {
                    $wh = " AND sd_secondarydataupload_t.region IN ('')";
                }
                //end
            } else {
                //others
                ...
                if (!empty($emp_zone)) {
                    $wh = "  AND sd_secondarydataupload_t.zone ='$emp_zone'";
                } else {
                    $wh = " AND sd_secondarydataupload_t.zone IN ('')";
                }
            }
...
GROUP BY
  f_year, month ORDER BY c_year, STR_TO_DATE(CONCAT(c_year, '-', month, '-01'), '%Y-%b-%d')");
            ...
            $month_sumchart_pre = $this->data['zone_sumchart_month'];

            $primData = [];

            foreach ($month_sumchart as $primary) {
                foreach ($month_sumchart_pre as $secondary) {
...
GROUP BY
  f_year, month, c_year ORDER BY c_year, STR_TO_DATE(CONCAT(c_year, '-', month, '-01'), '%Y-%b-%d')");
...
GROUP BY
  f_year, month, c_year ORDER BY c_year, STR_TO_DATE(CONCAT(c_year, '-', month, '-01'), '%Y-%b-%d')");
        ...
        return view('secondarysales&target.month', $this->data);
    }
...
GROUP BY
    f_year, zone");
            ...
            // distributor sale
            $this->data['all_ind_sal_reg'] = \DB::select("SELECT
    f_year,
    zone,
    c_year,
    ROUND(SUM(CASE WHEN type_d_s = 'DISTRIBUTOR' THEN sales_value ELSE 0 END), 0) as sale,
...
GROUP BY
    f_year, zone, c_year");
        ...
        return view('secondarysales&target.zone', $this->data);
    }
...
GROUP BY
    f_year, region, c_year");
        ...
        return view('secondarysales&target.region', $this->data);
    }
...
state,region,f_year");
        ...
        GROUP BY
        region,state,f_year HAVING (sale !='0' OR target !='0')
            UNION ALL
           SELECT
           f_year,region,state,
           ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' THEN purchase_stock_value ELSE 0 END), 0) as sale,
...
region,state,f_year");
        ...
        return view('secondarysales&target.state', $this->data);
    }
    ...
    public function DistributorWise(Request $request)
    {
        ...
        if ($zone != '' || $select_date != '' || $region != '' || $state != '') {
            ...
            // distributor sale
            $this->data['all_ind_sal_reg'] = \DB::select("SELECT
    state,f_year,stockist_dist_name as distributor,
   ROUND(SUM(CASE WHEN type_d_s = 'DISTRIBUTOR' THEN sales_value ELSE 0 END), 0) as sale
FROM
   sd_secondarydataupload_t
WHERE
...
GROUP BY
state,f_year,stockist_dist_name HAVING sale !='0'
    UNION ALL
   SELECT
   state,f_year,stockist_dist_name as distributor,
   ROUND(SUM(CASE WHEN type_d_s = 'DISTRIBUTOR' THEN sales_value ELSE 0 END), 0) as sale
...
state,f_year,stockist_dist_name HAVING sale !='0'");
        ...
        return view('secondarysales&target.distributor', $this->data);
    }
    ...
    public function HeadquatWise(Request $request)
    {
        ...
        if ($zone != '' || $select_date != '' || $region != '' || $state != '' || $area !='') {
            ...
            if ($region != '' && $state != '') {
                $condition = "AND region='$region' AND state='$state'";
            }
            // head q wise   
            // distributor sale
            $this->data['all_ind_sal'] = \DB::select("SELECT
            zone,region,state,hq_name,f_year,
               ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' THEN purchase_stock_value ELSE 0 END), 0) as sale,
                  ROUND(SUM(CASE WHEN data_type = '$cur_target' THEN purchase_stock_value ELSE 0 END), 0) as target
            FROM
               sd_secondarydataupload_t
    ...
    GROUP BY
    zone,region,state,hq_name,f_year HAVING (sale !='0' OR target !='0')");
        } else {
            //      default values
    ...
    zone,region,state,hq_name,f_year HAVING (sale !='0' OR target !='0')");
        ...
        return view('secondarysales&target.hqwise', $this->data);
    }
    ...
    public function PersonWise(Request $request)
    {
        ...
        if ($zone != '' || $select_date != '' || $region != ''  || $state != '') {
            ...
            $this->data['all_ind_sal'] = \DB::select("SELECT
    state,field_force_name as name,f_year,
   ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' THEN purchase_stock_value ELSE 0 END), 0) as sale,
   ROUND(SUM(CASE WHEN data_type = '$select_target' THEN purchase_stock_value ELSE 0 END), 0) as target
    FROM
       sd_secondarydataupload_t
   ...
   AND (
    ...
    WHERE
       ...
       AND (
        ...
        state,field_force_name,f_year");
            ...
            state,current_reporting_manager,field_force_name,
           month_y,f_year,c_year,
           ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' THEN purchase_stock_value ELSE 0 END), 0) as sale,
           ROUND(SUM(CASE WHEN data_type = '$cur_target' THEN purchase_stock_value ELSE 0 END), 0) as target
           FROM
           sd_secondarydataupload_t
           WHERE  f_year = '$fy_year' $condition $wh $notin
           AND STR_TO_DATE(CONCAT(c_year, '-', month, '-01'), '%Y-%b-%d') <= STR_TO_DATE('$select_year-$select_month-01', '%Y-%m-%d')
           GROUP BY
           state,current_reporting_manager,field_force_name,month_y,f_year HAVING (sale !='0' OR target !='0') AND field_force_name !='BIKASH DUTTA' ORDER BY  c_year, STR_TO_DATE(CONCAT(c_year, '-', month, '-01'), '%Y-%b-%d')");
    ...
    state,field_force_name,f_year ");
            ...
            $this->data['zone_wise'] = \DB::select("SELECT
            state,current_reporting_manager,field_force_name,month,
            month_y,f_year,c_year,
           ...
           FROM
           sd_secondarydataupload_t
           WHERE  f_year = '$cur_fyear'  $wh $notin
           AND STR_TO_DATE(CONCAT(c_year, '-', month, '-01'), '%Y-%b-%d') <= STR_TO_DATE('$last_y-$last_m-01', '%Y-%b-%d')
           GROUP BY
           state,current_reporting_manager,field_force_name,month,month_y,f_year HAVING (sale !='0' OR target !='0') AND field_force_name !='BIKASH DUTTA' ORDER BY  c_year, STR_TO_DATE(CONCAT(c_year, '-', month, '-01'), '%Y-%b-%d')");
        }

        return view('secondarysales&target.personwise', $this->data);
    }
    ...
    public function ProductWise(Request $request)
    {
        ...
        // if user select date search 
        if ($zone != '' || $select_date != '' || $region != '' || $division != '' || $state != '' || $area != '') {
            ...
            $this->data['all_ind_sal'] = \DB::select("SELECT
   division,product_form_change as name,f_year,
   ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' THEN purchase_stock ELSE 0 END), 0) as sale,
...
WHERE
   ...
   AND (
    ...
    GROUP BY
    division,product_form_change,f_year HAVING (sale !='0' OR target !='0')
        UNION ALL
       SELECT
       division,product_form_change as name,f_year,
       ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' THEN purchase_stock ELSE 0 END), 0) as sale,
    ...
    division,product_form_change,f_year HAVING (sale !='0' OR target !='0')");
        ...
        return view('secondarysales&target.productwise', $this->data);
    }
    ...
    public function Teamtarget(Request $request)
    {
        $wh = '';
        // hireachy query
        $depart = \Session::get('groupname');
        ...
        // if user select date search 
        if ($zone != '' || $select_date != '' || $region != '' || $state != '' || $manager != '' || $area != '') {
            ...
            // distributor sale
            $this->data['all_ind_sal'] = \DB::select("SELECT
 current_reporting_manager as name,hq_name,f_year,
   ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' THEN purchase_stock_value ELSE 0 END), 0) as sale,
   ROUND(SUM(CASE WHEN data_type = '$select_target' THEN purchase_stock_value ELSE 0 END), 0) as target
FROM
   sd_secondarydataupload_t
WHERE
   ...
   AND (
    ...
    GROUP BY
    current_reporting_manager,hq_name,f_year HAVING (sale !='0' OR target !='0')
        UNION ALL
       SELECT
       current_reporting_manager as name,hq_name,f_year,
       ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' THEN purchase_stock_value ELSE 0 END), 0) as sale,
    ...
    WHERE
       ...
       AND (
        ...
        current_reporting_manager,hq_name,f_year HAVING (sale !='0' OR target !='0')");
            ...
            // distributor sale
            $this->data['all_ind_sal'] = \DB::select("SELECT
    current_reporting_manager AS name,
    hq_name,
    f_year,
    ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' THEN purchase_stock_value ELSE 0 END), 0) AS sale,
    ROUND(SUM(CASE WHEN data_type = '$cur_target' THEN purchase_stock_value ELSE 0 END), 0) AS target
...
WHERE
    ...
    AND (STR_TO_DATE(CONCAT(c_year, '-', month, '-01'), '%Y-%b-%d') <= STR_TO_DATE('$last_y-$last_m-01', '%Y-%b-%d'))
GROUP BY
    current_reporting_manager, hq_name, f_year
...
FROM
    sd_secondarydataupload_t
WHERE
    f_year = '$pre_fyear'  $wh AND `hq_name` NOT LIKE '%dist'
    $notin AND (STR_TO_DATE(CONCAT(c_year, '-', month, '-01'), '%Y-%b-%d') <= STR_TO_DATE('$last_pre_y-$last_m-01', '%Y-%b-%d'))
GROUP BY
    current_reporting_manager, hq_name, f_year");
        }

        return view('secondarysales&target.teamtarget', $this->data);
    }


    // Distributor Close BAl Report

    public function Distcolbal(Request $request)
    {
        $wh = '';
        // hireachy query
        $depart = \Session::get('groupname');
        ...
        if ($zone != '' || $select_date != '' || $region != '' || $state) {
            ...
            $this->data['all_ind_sal'] = \DB::select("SELECT 
           zone,region,state,stockist_dist_name as name,f_year,
            ROUND(SUM(CASE WHEN type_d_s = 'DISTRIBUTOR' THEN closing_stock_value ELSE 0 END), 0) as sale
        FROM
            sd_secondarydataupload_t
        WHERE
       f_year='$fy_year' AND month_y ='$month_yr' $condition $notin
        GROUP BY 
        zone,region,state,stockist_dist_name,f_year HAVING sale !='0' ");
        } else {
        ...
        FROM
            sd_secondarydataupload_t
        WHERE
       f_year='$cur_fyear' AND month_y ='$month_y' $wh $notin
        GROUP BY 
        zone,region,state,stockist_dist_name ,f_year HAVING sale !='0'");
        }
        return view('secondarysales&target.disclobal', $this->data);
    }

    // Person wise Cloing Bal Report

    public function Personcolbal(Request $request)
    {
        ...
        // dd($month_yr);
        // if user select date search 
        if ($zone != '' || $select_date != '' || $region != '' || $state != '' || $manager != '') {
            ...
            $this->data['all_ind_sal'] = \DB::select("SELECT 
          current_reporting_manager as mangr,field_force_name as name,f_year,
            ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' THEN closing_stock_value ELSE 0 END), 0) as sale
        FROM
            sd_secondarydataupload_t
        WHERE
        month_y ='$month_yr' $condition $notin
        GROUP BY 
        current_reporting_manager,field_force_name,f_year HAVING sale !='0' ");
        } else {
        ...
        return view('secondarysales&target.perclobal', $this->data);
    }

    // Person wise payment out Report
    ...
    public function Paymentout(Request $request)
    {
        ...
        if ($zone != '' || $select_date != '' || $region != '' || $state != '') {
            ...
            $this->data['all_ind_sal'] = \DB::select("SELECT 
        zone,region,state,field_force_name as name,f_year,product_name,
            ROUND(SUM(CASE WHEN type_d_s = 'stockist' THEN opening_stock ELSE 0 END), 0) as sale
        FROM
            sd_secondarydataupload_t
        WHERE
         month_y ='$month_yr' $condition $notin AND product_name = 'payment outstanding'
        GROUP BY 
       zone,region,state,field_force_name,f_year HAVING sale !='0'");
        } else {
       ...
       zone,region,state,field_force_name,f_year HAVING sale !='0'");
        ...
        return view('secondarysales&target.paymentout', $this->data);
    }
    ...
    public function Salestrend(Request $request)
    {
        ...
        $this->data['divisions'] = \DB::select("SELECT DISTINCT division from `sd_secondarydataupload_t` where division!='-' $wh");
        $division = $request->input('division');
        $this->data['states'] = \DB::select("SELECT DISTINCT state from `sd_secondarydataupload_t` where 1=1 $wh");
        $state = $request->input('state');
        $this->data['stockists'] = \DB::select("SELECT DISTINCT stockist_dist_name from `sd_secondarydataupload_t` where type_d_s='STOCKIST'");
        $stock = $request->input('stock');
        $this->data['zones'] = \DB::select("SELECT DISTINCT zone from  `sd_secondarydataupload_t` where 1=1 $wh $notin");
        $zone = $request->input('zone');
        $this->data['regions'] = \DB::select("SELECT DISTINCT region from  `sd_secondarydataupload_t` where 1=1 $wh $notin");
        $region = $request->input('region');
        $this->data['hq_names'] = \DB::select("SELECT DISTINCT hq_name from  `sd_secondarydataupload_t` where 1=1 $wh $notin");
        $hq_name = $request->input('hq_name');
        $this->data['managers'] = \DB::select("SELECT current_reporting_manager AS name from `sd_secondarydataupload_t` where state NOT IN('EXPORT','LOCAL','GOVT SUPPLY') $wh");
        $manager = $request->input('manager');
        $this->data['distributors'] = \DB::select("SELECT DISTINCT stockist_dist_name as name from `sd_secondarydataupload_t` where type_d_s='DISTRIBUTOR'");
        ...
        $select_pre_year = ($select_pre - 1) . '-' . ($select_pre_y - 1);
        // if user select date search 
        if ($zone != '' || $select_date != '' || $region != '' || $stock != '' || $state != '' || $hq_name != '' || $dist_name != '' || $area != '') {
            ...
            if ($stock != '') {
                $condition = "AND stockist_dist_name='$stock'";
            }
            if ($state != '') {
                $condition = "AND state='$state'";
            }
            if ($hq_name != '') {
                $condition = "AND hq_name='$hq_name'";
            }
            if ($region != '') {
                $condition = "AND region='$region'";
            }
...
GROUP BY 
    zone,region,state,hq_name,month
HAVING 
    sale != 0
    UNION ALL
SELECT 
...
FROM
    sd_secondarydataupload_t
WHERE
    stockist_dist_name IN (
        SELECT stockist_dist_name
        FROM sd_secondarydataupload_t
        WHERE f_year = '$fy_year'
    )
    AND  f_year = '$select_pre_year'  $condition $wh $notin
...
GROUP BY 
    zone,region,state,stockist_dist_name, month
HAVING 
    sale != 0
    UNION ALL
SELECT 
...
c_year, STR_TO_DATE(CONCAT(c_year, '-', month, '-01'), '%Y-%b-%d')");
        ...
        return view('secondarysales&target.salandpurtrend', $this->data);
    }

    // Team target Report month

    public function Teamtargetmonth(Request $request)
    {
        $wh = '';
        // hireachy query
        $depart = \Session::get('groupname');
        ...
        $last_month = \DB::select("SELECT  month FROM `sd_secondarydataupload_t` ORDER BY `secondarydate_upload_id` DESC LIMIT 1");
        $l_dbyear = \DB::select("SELECT  c_year FROM `sd_secondarydataupload_t` ORDER BY `secondarydate_upload_id` DESC LIMIT 1");
        $last_m = $last_month[0]->month;
        $last_mon = date('m', strtotime($last_m));
        $last_y = $l_dbyear[0]->c_year;
        $last_mon = date('m', strtotime($last_m));
        $last_y = $l_dbyear[0]->c_year;
        ...
        if ($selected_month >= $fy_start_month) {
        ...
        if ($zone != '' || $select_date != '' || $region != '' || $state != '' || $manager != '') {
            ...
            $last_select_mon = ($select_month) . '\'' . $select_yr;
            $last_select_month = addslashes(($select_month) . '\'' . $select_yr);
            ...
            $this->data['mon_yr'] = $last_select_mon;
            $this->data['nxt_target'] = $last_target_mon;


            if ($select_date == '') {
                $last_select_month = $last_month_db;
                $next_month_target = $de_next_month_target;
                $this->data['mon_yr'] = $l_dbmon[0]->month_y;
                $this->data['nxt_target'] = $def_target_mon;
                $select_target = "IODP-E3 Target";
            }
            ...
            $this->data['all_ind_sal'] = \DB::select("SELECT
          current_reporting_manager AS name,
          field_force_name as field_name,
          f_year,
          ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' AND month_y = '$last_select_month' THEN purchase_stock_value ELSE 0 END), 0) AS sale,
          ROUND(SUM(CASE WHEN data_type = '$select_target' AND month_y = '$last_select_month' THEN purchase_stock_value ELSE 0 END), 0) AS target,
           ROUND(SUM(CASE WHEN data_type = '$select_target' AND month_y = '$next_month_target' THEN purchase_stock_value ELSE 0 END), 0) AS nxt_target
        ...
        field_force_name as field_name,
        f_year,
        ROUND(SUM(CASE WHEN type_d_s = 'STOCKIST' AND month_y = '$last_month_db' THEN purchase_stock_value ELSE 0 END), 0) AS sale,
        ROUND(SUM(CASE WHEN data_type = 'IODP-E3 Target' AND month_y = '$last_month_db' THEN purchase_stock_value ELSE 0 END), 0) AS target,
        ROUND(SUM(CASE WHEN data_type = 'IODP-E3 Target' AND month_y = '$de_next_month_target' THEN purchase_stock_value ELSE 0 END), 0) AS nxt_target
        ...
        return view('secondarysales&target.targetmonth', $this->data);
    }
}
