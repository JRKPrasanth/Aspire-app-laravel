...
{
        // track last login purpose - vignesh m
   public function getBrowserName($userAgent) {
    ...
    public function index($id=null)
    {
        ...
        $browser = $this->getBrowserName($userAgent);
         ...
         DB::table('tb_users')->where('id', $sid)->update([
...
$wh='';
$wh.=$this->accyearcondition('h.trx_date');
$das_access=array();
if(count($dsql)>0)
{
...
$this->data['das_access']=$das_access;
     ...
     $this->data['po_status']=json_encode($this->data['po_status']);
        ...
        foreach ($das_access as $val) {
           ...
           if ($val == 'MoreDashboards') {

           }

            /*   more dashboards  */
            if ($val == 'HRMSDashboard') {
            } else if ($val == 'SalesDashboard') {
            } else if ($val == 'QualityDashboard') {
            } else if ($val == 'PurchaseDashboard') {
            } else if ($val == 'OperationDashboard') {    
            } else if ($val == 'MaintenanceDashboard') {
            } else if ($val == 'AccountsDashboard') {
            }
    ...
    // sales return 
         ...
         } else if ($val == 'SalesReturnDirect') {
              $this->data['salesr_direct_count'] = \DB::select("SELECT COUNT(so_rma_hdr_id) as count FROM `so_rma_hdr_t` WHERE return_source='DIRECT' AND DATE(so_rma_hdr_t.return_date) BETWEEN '$grid_date' AND '$gridenddate'");
         } else if ($val == 'SalesReturnInvoice') {
              $this->data['salesr_invoice_count'] = \DB::select("SELECT COUNT(so_rma_hdr_id) as count FROM `so_rma_hdr_t` WHERE return_source='INVOICE' AND DATE(so_rma_hdr_t.return_date) BETWEEN '$grid_date' AND '$gridenddate'");
         } else if ($val == 'SalesReturnFromInvoice') {
                $this->data['sales_returninvoice_count'] = \DB::select("SELECT COUNT(*) AS count FROM (SELECT s_invoice_hdr_t.* FROM `s_invoice_hdr_t` LEFT JOIN s_dispatch_hdr_t ON s_dispatch_hdr_t.so_dispatch_hdr_id = s_invoice_hdr_t.reference_source_id WHERE s_invoice_hdr_t.invoice_status = 'APPROVED'
        AND s_dispatch_hdr_t.dispatch_status = 'SHIPPED' AND s_invoice_hdr_t.return_status = 0 UNION ALL SELECT s_invoice_hdr_t.* FROM `s_dispatch_hdr_t` LEFT JOIN s_invoice_hdr_t ON s_invoice_hdr_t.invoice_hdr_id = s_dispatch_hdr_t.reference_source_id WHERE s_dispatch_hdr_t.dispatch_source = 'INVOICE'
        AND dispatch_status = 'SHIPPED' AND s_invoice_hdr_t.return_status = 0) s_invoice_hdr_t JOIN m_customers_t ON m_customers_t.customer_id = s_invoice_hdr_t.`ship_to_customer_id` LEFT JOIN i_pricelist_hdr_t ON i_pricelist_hdr_t.pricelist_hdr_id = s_invoice_hdr_t.invoice_pricelist_id WHERE  DATE(s_invoice_hdr_t.invoice_date) BETWEEN '$grid_date' AND '$gridenddate'");
    ...
    $this->data['so_payment_pending']=\DB::select("SELECT invoice_number,invoice_date,(select m_customers_t.customer_name from m_customers_t where m_customers_t.customer_id=s_invoice_hdr_t.ship_to_customer_id) as customer_name,balance_amount,due_date,DATEDIFF(CURRENT_DATE(),due_date) as due  FROM `s_invoice_hdr_t` where invoice_status='APPROVED' and due_date<=CURRENT_DATE() and balance_amount>0 $wh");
     ...
     // sales return overall
           else if ($val == 'SalesReturnOverall') {

                $this->data['sales_return_overall'] = \DB::select("SELECT so_rma_hdr_t.so_rma_hdr_id,so_rma_hdr_t.rma_ref_no,so_rma_hdr_t.return_status,so_rma_hdr_t.reference_no,so_rma_hdr_t.return_source,so_rma_hdr_t.return_date,so_rma_hdr_t.total_amount,s_invoice_hdr_t.invoice_number,m_customers_t.customer_name FROM `so_rma_hdr_t` left join s_invoice_hdr_t on s_invoice_hdr_t.invoice_hdr_id=so_rma_hdr_t.reference_source_id left join m_customers_t on m_customers_t.customer_id=so_rma_hdr_t.customerid where 1=1 AND DATE(so_rma_hdr_t.return_date) BETWEEN '$grid_date' AND '$gridenddate' ORDER BY so_rma_hdr_t.so_rma_hdr_id desc");
   ...
   // dd($overall_production_qty);
    ...
    else if($val=="SubinventoryTransferReceive-FinishedGoods"){
            $this->data['si_finishgoods']=\DB::select("SELECT COUNT(*) AS count FROM i_subinventory_transfer_lines_t AS i LEFT JOIN i_subinventory_transfer_hdr_t AS h ON i.subinventory_transfer_hdr_id = h.subinventory_transfer_hdr_id LEFT JOIN m_product_groups_t AS pg ON i.product_group_id = pg.product_group_id LEFT JOIN m_products_t AS p ON i.product_id = p.product_id LEFT JOIN m_subinventory_t AS frm_subinv ON frm_subinv.subinventory_id = h.frm_subinv_id LEFT JOIN m_subinventory_t AS to_subinv ON to_subinv.subinventory_id = h.to_subinv_id LEFT JOIN tb_users AS u ON u.id = i.created_by WHERE  i.status != 'APPROVED' $wh AND h.to_subinv_id ='3'");
     }else if($val=="SubinventoryTransferReceive-SemiGoods"){
            $this->data['si_semigoods']=\DB::select("SELECT COUNT(*) AS count FROM i_subinventory_transfer_lines_t AS i LEFT JOIN i_subinventory_transfer_hdr_t AS h ON i.subinventory_transfer_hdr_id = h.subinventory_transfer_hdr_id LEFT JOIN m_product_groups_t AS pg ON i.product_group_id = pg.product_group_id LEFT JOIN m_products_t AS p ON i.product_id = p.product_id LEFT JOIN m_subinventory_t AS frm_subinv ON frm_subinv.subinventory_id = h.frm_subinv_id LEFT JOIN m_subinventory_t AS to_subinv ON to_subinv.subinventory_id = h.to_subinv_id LEFT JOIN tb_users AS u ON u.id = i.created_by WHERE  i.status != 'APPROVED' $wh AND h.to_subinv_id ='5'");
     }else if($val=="SubinventoryTransferReceive-SampleGoods"){
            $this->data['si_samplegoods']=\DB::select("SELECT COUNT(*) AS count FROM i_subinventory_transfer_lines_t AS i LEFT JOIN i_subinventory_transfer_hdr_t AS h ON i.subinventory_transfer_hdr_id = h.subinventory_transfer_hdr_id LEFT JOIN m_product_groups_t AS pg ON i.product_group_id = pg.product_group_id LEFT JOIN m_products_t AS p ON i.product_id = p.product_id LEFT JOIN m_subinventory_t AS frm_subinv ON frm_subinv.subinventory_id = h.frm_subinv_id LEFT JOIN m_subinventory_t AS to_subinv ON to_subinv.subinventory_id = h.to_subinv_id LEFT JOIN tb_users AS u ON u.id = i.created_by WHERE  i.status != 'APPROVED' $wh AND h.to_subinv_id ='4'");
     } else if($val=="SubinventoryTransferReceive-Store"){
            $this->data['si_store']=\DB::select("SELECT COUNT(*) AS count FROM i_subinventory_transfer_lines_t AS i LEFT JOIN i_subinventory_transfer_hdr_t AS h ON i.subinventory_transfer_hdr_id = h.subinventory_transfer_hdr_id LEFT JOIN m_product_groups_t AS pg ON i.product_group_id = pg.product_group_id LEFT JOIN m_products_t AS p ON i.product_id = p.product_id LEFT JOIN m_subinventory_t AS frm_subinv ON frm_subinv.subinventory_id = h.frm_subinv_id LEFT JOIN m_subinventory_t AS to_subinv ON to_subinv.subinventory_id = h.to_subinv_id LEFT JOIN tb_users AS u ON u.id = i.created_by WHERE  i.status != 'APPROVED' $wh AND h.to_subinv_id ='6'");
     }else if($val=="SubinventoryTransferReceive-accessories"){
        $this->data['si_acc']=\DB::select("SELECT COUNT(*) AS count FROM i_subinventory_transfer_lines_t AS i LEFT JOIN i_subinventory_transfer_hdr_t AS h ON i.subinventory_transfer_hdr_id = h.subinventory_transfer_hdr_id LEFT JOIN m_product_groups_t AS pg ON i.product_group_id = pg.product_group_id LEFT JOIN m_products_t AS p ON i.product_id = p.product_id LEFT JOIN m_subinventory_t AS frm_subinv ON frm_subinv.subinventory_id = h.frm_subinv_id LEFT JOIN m_subinventory_t AS to_subinv ON to_subinv.subinventory_id = h.to_subinv_id LEFT JOIN tb_users AS u ON u.id = i.created_by WHERE  i.status != 'APPROVED' $wh AND h.to_subinv_id ='7'");
    } else if($val=="SubinventoryTransferReceive-lab"){
        $this->data['si_lab']=\DB::select("SELECT COUNT(*) AS count FROM i_subinventory_transfer_lines_t AS i LEFT JOIN i_subinventory_transfer_hdr_t AS h ON i.subinventory_transfer_hdr_id = h.subinventory_transfer_hdr_id LEFT JOIN m_product_groups_t AS pg ON i.product_group_id = pg.product_group_id LEFT JOIN m_products_t AS p ON i.product_id = p.product_id LEFT JOIN m_subinventory_t AS frm_subinv ON frm_subinv.subinventory_id = h.frm_subinv_id LEFT JOIN m_subinventory_t AS to_subinv ON to_subinv.subinventory_id = h.to_subinv_id LEFT JOIN tb_users AS u ON u.id = i.created_by WHERE  i.status != 'APPROVED' $wh AND h.to_subinv_id ='8'");
    }
         ...
         /*Leave Balance Count*/
                    $this->data['cl_balance_count']=\DB::select("SELECT causal_leave  FROM Leave_balance_tbl WHERE employee_id = $eid");
     }else if($val=="ELBalance"){
                    $this->data['el_balance_count']=\DB::select("SELECT earn_leave  FROM Leave_balance_tbl WHERE employee_id = $eid");
     }else if($val=="Comp-OffBalance"){
                    $this->data['compoff_balance_count']=\DB::select("SELECT comp_off_leave  FROM Leave_balance_tbl WHERE employee_id = $eid");
     }else if($val=="LeaveApproval"){
                    $this->data['leave_initiated']=\DB::select("SELECT COUNT(leave_id) as count FROM `hr_leaves_t` WHERE employee_id=$eid AND leave_status='INITIATED'");
     }else if($val=="LeaveApprovePending"){
                    $this->data['report_approve_pending']=\DB::select("SELECT COUNT(leave_id) as count FROM `hr_leaves_t` WHERE forwarded_id=$eid AND leave_status='INITIATED'");
...
WHERE
    1 = 1");
            ...
            $this->data['rol_rm']=\DB::select("SELECT
    *
FROM
    (
    SELECT
...
WHERE
    m_products_t.product_group_id = 2 AND p_po_hdr_t.po_status != 'CLOSED' AND p_po_hdr_t.po_status != 'COMPLETED' AND p_po_hdr_t.po_status != 'CANCELLED' AND p_po_hdr_t.po_status != 'REJECTED'
GROUP BY
    m_products_t.product_id
) f
LEFT JOIN m_product_category_t ON m_product_category_t.product_category_id = f.product_category_id
...
WHERE
    1 = 1");
            ...
            $this->data['rol_pm']=\DB::select("SELECT
    *
FROM
    (
    SELECT
       ...
       ( CASE WHEN SUM(Stock - min_order_qty) > 0 THEN CASE WHEN min_order_qty > f.re_order_level THEN min_order_qty else f.re_order_level END WHEN SUM(Stock - min_order_qty) < 0 THEN CASE WHEN min_order_qty > f.re_order_level THEN min_order_qty else f.re_order_level END else 
        ...
        sum(POQty) as POQty,
         (CASE WHEN SUM(re_order_level - POQty) > 0 AND ROUND(SUM((Stock - min_order_qty)* (-1)),2) > 0 THEN 
          
          CASE WHEN min_order_qty > f.re_order_level THEN SUM(min_order_qty - POQty) ELSE SUM(f.re_order_level - POQty) END ELSE 0 END) as yet_to_order
...
GROUP BY
    ...
    m_products_t.product_id
        ...
        SELECT
            ...
            COALESCE(
                (
                    CASE WHEN SUM(i_qoh_detail_t.qoh_trx_qty) > 0 THEN ROUND(SUM(i_qoh_detail_t.qoh_trx_qty),2) ELSE 0
                END
            ),
            0
    ) AS Stock,
...
LEFT JOIN p_po_hdr_t ON p_po_hdr_t.po_hdr_id = p_po_lines_t.po_hdr_id 
WHERE
    m_products_t.product_group_id = 3 AND p_po_hdr_t.po_status != 'CLOSED' AND p_po_hdr_t.po_status != 'COMPLETED' AND p_po_hdr_t.po_status != 'CANCELLED' AND p_po_hdr_t.po_status != 'REJECTED'
GROUP BY
    m_products_t.product_id
...
WHERE
    1 = 1");
				...
				// purpose for Announcement popup 
				$today = date('Y-m-d');

            $this->data['announcements'] = DB::table('hr_announcement_t')
