...
class ProductionproductrptController extends Controller
{
    ...
    if (!empty($start_date)) {
                 ...
                 $this->data['product_summary'] = \DB::select("select emp.qa_assigned_name,emp.product_name,emp.yr_month,SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(emp.endtime, emp.starttime)))) AS  wrk_hrs,
...
(
    ...
    WHERE
        1 = 1  AND emp.job_date BETWEEN '$start_date' AND '$end_date' $wh  AND emp.group_type='8' GROUP BY emp.qa_assigned_name,emp.product_name,emp.yr_month ORDER BY SUBSTRING(yr_month, -2) + 0 ASC, STR_TO_DATE(CONCAT('01-', yr_month), '%d-%b-%y') ASC");
...
(
    ...
    WHERE
        1 = 1  AND emp.job_date BETWEEN '$start_date' AND '$end_date' $wh AND emp.group_type='8' GROUP BY emp.machine_name,emp.yr_month ORDER BY SUBSTRING(yr_month, -2) + 0 ASC, STR_TO_DATE(CONCAT('01-', yr_month), '%d-%b-%y') ASC");
        
        // target wise report
        
        $this->data['target_summary'] = \DB::select("select emp.job_assigned_name,emp.machine_name,emp.product_name,emp.emp_qty,emp.range_to,emp.yr_month,SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(emp.endtime, emp.starttime)))) AS  wrk_hrs,sec_to_time(sum(time_to_sec(MAKETIME(LPAD(SUBSTRING_INDEX(emp.std_hrs, '.', 1),2,0),
    RPAD(SUBSTRING_INDEX(emp.std_hrs, '.', -1),2,0),00)))) as std_hrs,        SEC_TO_TIME(
        SUM(TIME_TO_SEC(MAKETIME(
            LPAD(SUBSTRING_INDEX(emp.std_hrs, '.', 1), 2, '0'),
            RPAD(SUBSTRING_INDEX(emp.std_hrs, '.', -1), 2, '0'),
            0
        ))) -
        SUM(TIME_TO_SEC(TIMEDIFF(emp.endtime, emp.starttime)))
    ) AS hrs_diff  from (SELECT *
    FROM
        (
        SELECT
            t.*,ROUND(((t.actualhrs/t.range_to) * t.job_qty),2) as machour,
...
(
    ...
    WHERE
        1 = 1  AND emp.job_date BETWEEN '$start_date' AND '$end_date' $wh AND emp.group_type='8' GROUP BY emp.job_assigned_name,emp.machine_name,emp.product_name,emp.yr_month ORDER BY SUBSTRING(yr_month, -2) + 0 ASC, STR_TO_DATE(CONCAT('01-', yr_month), '%d-%b-%y') ASC");
        
  }
        return view('productiondashboard.productrpt', $this->data);
   ...
   public function Producttype(Request $request){
        ...
        $daily_summary = \DB::select("select emp.machine_name,emp.emp_qty,date(emp.job_date) as date,CONCAT(LEFT(MONTHNAME(date(emp.job_date)),3), '-', year(date(emp.job_date))) as month_yr,SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(emp.endtime, emp.starttime)))) AS  wrk_hrs from (SELECT *
    FROM
        (
        SELECT
            t.*,ROUND(((t.actualhrs/t.range_to) * t.job_qty),2) as machour,
...
(
    ...
    WHERE
       ...
       1 = 1 AND emp.job_date BETWEEN '$start_date' AND '$end_date' 
         AND emp.job_assigned_name ='$emp_name' AND emp.product_name ='$product' GROUP BY emp.machine_name,emp.job_date order by emp.job_date ASC");
...
foreach ($daily_summary as $value) {
    ...
    if (!isset($uniqueMonthYears[$monthYear])) {
        ...
        $htmlTable .= '<th class="text-center bg-danger-subtle">Machine Name</th>';
...
$(document).ready(function(){
    ...
    if($product !=''){
        ...
        $wh = " AND emp.p_product_id ='$product'";
        
    }else{
        
        $wh = " AND emp.type_id = '$prd_type'";
    }
    

        $daily_summary = \DB::select("select emp.job_assigned_name,emp.product_name,emp.emp_qty,date(emp.job_date) as date,CONCAT(LEFT(MONTHNAME(date(emp.job_date)),3), '-', year(date(emp.job_date))) as month_yr,SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(emp.endtime, emp.starttime)))) AS  wrk_hrs from (SELECT *
    FROM
        (
        SELECT
            t.*,ROUND(((t.actualhrs/t.range_to) * t.job_qty),2) as machour,
...
(
 ...
 AND emp.machine_name ='$machine' $wh GROUP BY emp.job_assigned_name,emp.product_name,emp.job_date order by emp.job_date ASC");
//dd($daily_summary);
