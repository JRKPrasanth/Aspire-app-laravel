...
class GeneratepayrollController extends Controller
{
    ...
    public function hrmsjournal()
    {
        ...
        $prevoius_emp_id = 0;
        $nr = "<style>table, td, th {
      border: 1px solid black;
    }</style><table style='border:solid 1px #000'><thead><tr  style='border:solid 1px #000'><th>Employee Number</th><th>Department</th><th>Allowances</th></tr></thead><tbody>";
        foreach ($employee_data as $employee) {
            ...
            $department = \DB::table('m_department_lines_t')->where('department_line_id', $dep)->get();
            ...
            $allowance_col = array_column($accounts_data_allowance, 'allowance_id');
            ...
            foreach ($allowance_deduction as $i => $v) {
                foreach ($v as $ii => $vv) {
                    ...
                    if (!in_array($ii, $allowance_col)) {
                        $allow = \DB::table('m_allowance_tbl')->where('allowance_id', $ii)->get();
                        // dd($allow);
                        $run = 0;
                        $nr_qllow_ids[] = $ii;
                        $nr_allow .= $allow[0]->allowance_name . '(' . $ii . ')<br>';
                    }
            ...
            if ($run == 1) {
                ...
                foreach ($data_acc_debit as $k1 => $v1) {
                    ...
                    if ($data_acc_value_debit[$k1] > 0) {
                        ...
                        $journal_lines_data[$tkey]['company_id'] = "1";
                ...
                $not_run['emp_number'][] = $employee->emp_number;
                $not_run['department'][] = $department[0]->department_line_id;
                $not_run['allowance_id'][] = json_encode($nr_qllow_ids);
                $nr .= "<tr  style='border:solid 1px #000'><td>" . $employee->emp_number . "</td><td>" . $department[0]->sub_department_name . "</td><td>" . $nr_allow . "</td></tr>";
                                            ...
                                            if (count($payroll_exists) == 0) {
                                                ...
                                                if (count($attendence_setting) > 0) {
                                                    ...
                                                    if (count($shift_type) > 0) {
                                                        ...
                                                        $basic_pay = $grade_details[0]->basic_pay;
                                                        $effects_from = $grade_details[0]->effects_from;
                                                        $bonus_amount = $grade_details[0]->bonus_amount;
                                                        $hra_pay = $grade_details[0]->hra;
                                                        $da_pay = $grade_details[0]->da;
                                                        $gratuity_pay = $grade_details[0]->gratuity;
                                                        $esi_pay = $grade_details[0]->esi;
                                                        $pf_pay = $grade_details[0]->pf;
                                                        $pt_pay = $grade_details[0]->pt;
                                                        $gross_pay = $grade_details[0]->gross_pay;
                                                        $net_pay = $grade_details[0]->net_pay;
                                                        $welfare_fund_pay = $grade_details[0]->welfare_fund;
                                                        $allowance_pay = $grade_details[0]->allowance;
                                                        $volunter_pf_pay = $grade_details[0]->volunter_pf;
                                                        $esi_amount_pay = $grade_details[0]->esi_amount;
                                                        $pf_amount_pay = $grade_details[0]->pf_amount;
                                                        $pt_amount_pay = $grade_details[0]->pt_amount;
                                                        ...
                                                        if ($grade_details[0]->pf == 1) {
                                                            if (count($pf_detatil) > 0) {
                                                                ...
                                                                $com_con1 = round($com_con1);
                                                                $com_con2 = round($com_con2);
                                                                ...
                                                                $company_contribute['amount'] = $com_con1;
                                                                $company_contribute['amount1'] = $com_con2;
                                                        ...
                                                        $deduction = DB::table("hr_employee_advance_t")->where('employee_id', $emp_id)->where('remaining_amount', '!=', 0)->where('effective_date', '<=', $curr_date)->where('paid_status', 0)->where('approved_status', '=', 1)->get();
                                                        ...
                                                        $data = array(
                                                            ...
                                                            'sl' => $sl,
                                                            'ot' => $ot_amount,
                                                            'basic_pay' => round($basic_pay),
                                                            'annual_allowance_pay' => round($annual_allowance),
                                                            'effects_from_pay' => round($effects_from),
                                                            'bonus_amount_pay' => round($bonus_amount),
                                                            'hra_pay' => round($hra_pay),
                                                            'da_pay' => round($da_pay),
                                                            'gratuity_pay' => round($gratuity_pay),
                                                            'esi_pay' => round($esi_pay),
                                                            'pf_pay' => round($pf_pay),
                                                            'pt_pay' => round($pt_pay),
                                                            'gross_pay' => round($gross_pay),
                                                            'net_pay' => round($net_pay),
                                                            'welfare_fund_pay' => round($welfare_fund_pay),
                                                            'allowance_pay' => $allowance_pay,
                                                            'volunter_pf_pay' => round($volunter_pf_pay),
                                                            'esi_amount_pay' => round($esi_amount_pay),
                                                            'pf_amount_pay' => round($pf_amount_pay),
                                                            'pt_amount_pay' => round($pt_amount_pay),
                                                            'partial_day' => $partial,
