...
{   
    ...
    public function payrollgenerate() 
    {
        ...
        $months  = DB::table('month')->where('id',$_GET['month'])->get();
        ...
        if(count($cutoff_date)>0)
        {
                ...
                {
                        $cuttoff_start_date = $cut_off[2] + 1;
                        $start_month        = $month-1;
                }
                ...
                {	
                        ...
                        $cutoff_date    = date($current_year.'-'.($start_month-1).'-'.$cut_off[2]);
                ...
                else
                {
                      ...
                      $cutoff_date       = date($current_year.'-'.$start_month.'-'.$cut_off[2]);
                     $start_date       = date($current_year.'-'.$month.'-'.$cuttoff_start_date);
                }
            ...
            { 	
				...
				foreach($employees as $key=>$employee) 
				{
				   ...
				   $department_check        = json_decode($employee->department);
                                   if (in_array($department, $department_check))
                                   {
				...
				$payroll_exists = DB::table("hr_employee_payroll_lists")->where('month',$month)->where('year',$current_year)->where('employee_id',$emp_id)->get();				
			        // check attendance exists or not
                                $attendance_exists_employee  =   DB::table("hr_emp_attendence")->whereMonth('atten_date',$month)->whereYear('atten_date',$current_year)->where('emp_id',$emp_code)->get();
                                $ot_settings  =   DB::table("m_ot_t")->where('employee_type',$emp_type)->where('grade_id',$employee_position)->where('position_id',$employee->job_title)->where('active','Yes')->get();
                ...
                ->select('m_emp_esi.limitto','m_emp_esi.employeer_contribute','m_emp_esi.company_contribute','m_emp_esi.company_contribute1')->where('lookup_code','PF')->get();
						...
						if($grade_details[0]->esi==1)
						{
							...
							$company_contribute['amount']         =   round($com_con);                                             
							$company_contribute['amount_employee']=  round($emp_con);                                             
                                                    ...
                                                    if(count($pf_detatil)>0){
                                                        $limit=$pf_detatil[0]->limitto;
                                                            if($pf_value>=$limit){
                                                                ...
                                                                $com_con2                                = $pf_value*$pf_detatil[0]->company_contribute1/100;
                                                            }
															$epf=$pf;
							 $pf =$volunter_pf+$pf;
  ...
  //dd("volunter--".$volunter_account."basic--".$basic_account."hra--".$hra_account."da--".$da_account."net--".$net_account."esi_emp--".$esi_employee_account."esi_comp--".$esi_company_account."pf_emp--".$pf_employee_account."pf_cmp--".$pf_company_account."pf_cmp1--".$pf_company1_account."pt--".$pt_account."tax--".$tax_account."ot--".$ot_account."annual allow--".$annual_allowance_account."gratuity--".$gratuity_account);
       ...
       $journal_name="PAYROLL-".$account_month;
...
if($data_acc_value_exp[$k1] > 0)
{
				...
				else
				{
                                    ...
                                    return array('status' => 'error','message' =>"Please assign Employee to these Department");  
