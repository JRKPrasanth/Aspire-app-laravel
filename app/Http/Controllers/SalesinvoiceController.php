...
class SalesinvoiceController extends Controller
{
    ...
    public function getSalesinvoiceData()
    {
        ...
        if ($depart == "14") {
            ...
            $wh .= " and s_invoice_hdr_t.ship_to_customer_id in (" . $cus_id[0]->dis . ")";
        }
        ...
        s_invoice_hdr_t.doc_attachment,
        ...
        CASE WHEN b.first_name = A.first_name  AND s_invoice_hdr_t.invoice_status != 'APPROVED' THEN
                                'Yet to Approve'
                                ELSE
                                A.first_name
        ...
        left join hr_employee_t A on (A.employee_id = s_invoice_hdr_t.last_updated_by)
        left join hr_employee_t b on (b.employee_id = s_invoice_hdr_t.created_by)
    ...
    m_frieghtcarriers_hdr_t.ar_frieghtcarriers_hdr_id=s_invoice_hdr_t.ar_frieghtcarriers_hdr_id)    
        ...
        where 1=1 $wh order by s_invoice_hdr_t.invoice_hdr_id DESC ) AS v1";
    ...
    public function replace($id = null, $invtype = null, $labid = null)
    {
        ...
        foreach ($tablelines as $key => $value) {
            ...
            if ($table[0]->ship_to_customer_id == "0" && $value->product_id == "0") {
            ...
            } else if ($table[0]->ship_to_customer_id != "0" && $value->product_id == "0") {
    ...
    public function create($id = null, $invtype = null, $labid = null)
    {
        ...
        if ($id == '0') {
            ...
            if ($invtype != 'EXPORT INVOICE') {
                $this->data['invoice_currency'] = $this->jCombo('f_account_currency_t', 'account_currency_id', 'currency_code', '37');
            } else {
                $this->data['invoice_currency'] = $this->jCombo('f_account_currency_t', 'account_currency_id', 'currency_code', '');
            }
            ...
            $this->data['con_exc_rate'] = $table[0]->con_exc_rate;
            ...
            $this->data['schemes'] = $this->jcustommultiselect('s_schemes_hdr_t', 'schemes_hdr_id', 'schemes_name', $schemes, ' and savestatus="APPROVED" and approvestatus="APPROVED"');
            ...
            $this->data['cash_schemes'] = $this->jcustommultiselect('s_schemes_hdr_t', 'schemes_hdr_id', 'schemes_name', $cash_schemes, ' and savestatus="APPROVED" and approvestatus="APPROVED"');
            ...
            if ($table[0]->invoice_type == "SAMPLE") {
                $table_emp = \DB::table('s_invoice_hdr_t')->where('invoice_hdr_id', $id)->get();
                //  dd($table_emp[0]->employee_id);
                $emp = \DB::table('hr_emp_contact')->select('hr_emp_contact.current_street', 'hr_emp_contact.current_street_address', 'hr_emp_contact.current_postal_code', 'm_countries_t.country_name', 'm_states_t.state_name', 'm_cities_t.city_name')->leftjoin('m_countries_t', 'hr_emp_contact.current_country', "=", 'm_countries_t.country_id')->leftjoin('m_states_t', 'hr_emp_contact.current_state', "=", 'm_states_t.state_id')->leftjoin('m_cities_t', 'hr_emp_contact.current_city', "=", 'm_cities_t.city_id')->where('employee_id', $table_emp[0]->employee_id)->get();
                //dd($emp);
                if (count($emp) > 0) {
    ...
    public function save(Request $request)
    {
            ...
            if ($_POST['source'] == "SALES ORDER" || ($_POST['source'] == "DISPATCH" && $_POST['ar_sales_hdr_id'] != '')) {
                ...
                foreach ($_POST['bulk_product_id'] as $k => $v) {
                    ...
                    foreach ($qty_up_data as $k0 => $v0) {
                        ...
                        $query_invoice = DB::table('s_salesorder_lines_t')
                            ->where('sales_hdr_id', $v0)
                            ->where('product_id', $v)
                            ->first();
                        ...
                        if ($query_invoice && $query_invoice->invoiced_qty > 0) {
                            $inv_qty = $query_invoice->invoiced_qty + $qty_up[$k0];
                        } else {
                            $inv_qty = $qty_up[$k0];
                        }
        ...
        try {
            ...
            if ($status == "APPROVED") {
                ...
                if (isset($id)) {
                    ...
                    if ($_POST['invoice_currency'] == 37) {
                        ...
                        $data = \DB::table('f_account_exchangerates_t')->whereDate('from_date', '<=', $invdate)->whereDate('to_date', '>=', $invdate)->where('from_currency_id', $_POST['invoice_currency'])->where('to_currency_id', 37)->where('active', 'Yes')->where('company_id', $compy)->select('*')->first();

                        if (!empty($data)) {

                            $convertion_value = $data->conversion_rate ?? 1;
                        } else {
                    ...
                    $custable = \DB::table('m_customers_t')->join('m_customer_types_t', 'm_customer_types_t.customer_type_id', '=', 'm_customers_t.customer_type_id')->where('customer_id', '=', $custid)->select('m_customers_t.account_structure_id', 'm_customer_types_t.account_id')->first();

                    $account_structure_id = $custable->account_structure_id ?? 0;


                    $accntsettings = DB::table('f_account_setting_t')->where('module_name', 'salesaccount')->first();
                    $roundoff = $_POST['round_off'];
                    $sales_account_id = $custable->account_id ?? 0;
                    $cogs_account_id = $accntsettings->cogs_account_id ?? 0;
                    $tkey = 0;
                    ...
                    $journal_lines_data[$tkey]['debit_amount'] = ((float) ($_POST['invoice_grand_total'] ?? 0) * (float) $convertion_value) + ((float) ($roundoff ?? 0) * (float) $convertion_value);
                    ...
                    foreach ($_POST['bulk_product_id'] as $key => $value) {
                        $disc_acc_code = \DB::table('m_products_t')->where('product_id', '=', $value)->select('disc_account_code')->first();
                        $disc_acc_id = $disc_acc_code->disc_account_code ?? 0;
                        $qty = $_POST['bulk_qty'][$key];
                        if (($_POST['bulk_discount_amount'][$key] != '' && $_POST['bulk_discount_amount'][$key] > 0)) {
                            ...
                            $journal_lines_data[$tkey]['batch_number'] = $_POST['bulk_batch_number'][$key];
                    ...
                    if (($_POST['trade_discount_pre'] != '' && $_POST['trade_discount_pre'] > 0)) {
                        $disc_acc_code = \DB::table('f_account_setting_t')->where('module_name', '=', 'salesaccount')->select('domestic_account_id')->first();
                        $disc_acc_id = $disc_acc_code->domestic_account_id ?? 0;

                        foreach ($_POST['bulk_product_id'] as $key => $value) {
                            $tkey++;
                            $qty1 = $_POST['bulk_qty'][$key];
                            $qty = (float) ($_POST['bulk_qty'][$key] ?? 0);
                            $price = (float) ($_POST['bulk_unit_price'][$key] ?? 0);
                            ...
                            $journal_lines_data[$tkey]['reference_id'] = $value;
                            $journal_lines_data[$tkey]['batch_number'] = $_POST['bulk_batch_number'][$key];
                            $journal_lines_data[$tkey]['product_qty'] = $qty1;
                            $journal_lines_data[$tkey]['account_id'] = $disc_acc_id;
                    ...
                    foreach ($_POST['bulk_product_id'] as $key => $value) {
                        ...
                        $journal_lines_data[$tkey]['reference_id'] = $value;
                        $journal_lines_data[$tkey]['batch_number'] = $_POST['bulk_batch_number'][$key];
                        $journal_lines_data[$tkey]['product_qty'] = $qty2;
                    ...
                    if ($_POST['tcs_applicable'] != "NO") {
                        ...
                        $journal_lines_data[$tkey]['batch_number'] = '';
                        $journal_lines_data[$tkey]['product_qty'] = '';
                        $journal_lines_data[$tkey]['account_id'] = $_POST['tcs_account_id'];
                        $journal_lines_data[$tkey]['debit_amount'] = '';
                        $journal_lines_data[$tkey]['credit_amount'] = $_POST['tcs_amount'] * $convertion_value;
                        $journal_lines_data[$tkey]['line_no'] = $tkey + 1;
                        $journal_lines_data[$tkey]['created_by'] = \Session::get('id');
                        $journal_lines_data[$tkey]['created_at'] = date('Y-m-d H:i:s');
                        $journal_lines_data[$tkey]['last_updated_by'] = \Session::get('id');
                    ...
                    if ($round_off != "" && $round_off != 0) {
                        ...
                        if ($round_off > 0) {
                            $roundoffvalpls = 0;
                            $roundoffvalles = Round($round_off, 2) * $convertion_value;
                        } else {
                            $roundoffvalpls = Round((-1) * ($round_off), 2) * $convertion_value;
                            $roundoffvalles = 0;
                        }
                        $tkey++;
                        $journal_lines_data[$tkey]['journal_entry_id'] = $jid;
            ...
            if ($request->invoice_status == "INITIATED") {
                $noti_msg = "Sales Invoice" . $request->invoice_number . " Initiated";
            ...
            if ($request->invoice_status == "APPROVED") {
                $noti_msg = "Sales Invoice" . $request->invoice_number . " Approved";
    ...
    public function getPrint($id = null, $invoice = null)
    {
        ...
        $customer = DB::table('s_invoice_hdr_t')->leftjoin('m_customers_t', 'm_customers_t.customer_id', '=', 's_invoice_hdr_t.ship_to_customer_id')
            ...
            ->select(
                ...
                's_invoice_hdr_t.tcs_applicable',
        ...
        if (isset($id)) {
            $currency_type = DB::table('s_invoice_hdr_t')->leftjoin('f_account_currency_t', 'f_account_currency_t.account_currency_id', '=', 's_invoice_hdr_t.invoice_currency')
                ->select('f_account_currency_t.currency_code')->where('invoice_hdr_id', $id)->get();
        ...
        if (!empty($customer)) {
            ...
            $this->data['tcs_amount'] = $customer[0]->tcs_amount;
            ...
            $this->data['round_off'] = "";
            $this->data['tcs_amount'] = "";
            $this->data['tcs_applicable'] = "";
        ...
        if ($bill_to_ship_address != 0) {
            foreach ($bill_to_ship_address as $key => $value) {
                ...
                } else if ($value->site_type == "BILL_TO") {
                    // dd($value->address);
                    $this->data['address'] = $value->address;
                    $this->data['city'] = $this->getCity($value->city);
                    //$this->data['city_bill']=$this->data['city'][0]->city_name;
        ...
        //$this->data['company_logo'] = 'backend-logo.jpg';
        /*************************** End *********************************/
        ...
        $sql = \DB::SELECT("select * from s_invoice_lines_t  WHERE invoice_line_id='" . $lid . "' group by hsn_code");
        ...
        if ($customer[0]->source == ("DISPATCH" || "REPLACEMENT" || "EXPORT INVOICE")) {
            $lines = \DB::table('s_invoice_lines_t')
                ...
                ->orderBy('s_invoice_lines_t.invoice_line_id', 'asc')
                ->get();
        }
        //dd($lines);
        ...
        foreach ($this->data['subgrid'] as $ke => $value) {
            ...
            if ($value->comments != '') {
                $polines[$key]['comments'] = "(Note: " . $value->comments . ")";
            } else {
                $polines[$key]['comments'] = "";
            }
	...
	.assembly_product_id=' . $value->product_id . ' and m_products_t.product_group_id="1"');
                ...
                if ($arr['product_group_id'] != '12') {
                    $polines[$key]['batch_mfg'] = $polines[$key]['batch_mfg'] . " HSN:" . $polines[$key]['hsn_code'] . " " . $polines[$key]['gst_per'];
                } else {
            ...
            if ($polines[$key]['gst_tax'] == "28" || $polines[$key]['gst_tax'] == "18") {
                $X += $value->tax_amount;
                $tot_tax_amt_x = $X / 2;
            } else {
                $Y += $value->tax_amount;
            ...
            $polines[$key]['free_qty'] = $value->free_qty;
            ...
            $line_total = $value->unit_price * $value->qty;
            $total += $line_total;

            // Calculate discount - use discount_amount if set, otherwise use percentage
            if ($value->discount_amount > 0) {
                $discount = $value->discount_amount;
            } else {
                $discount = $line_total * ($value->discount_percentage / 100);
            }

            // Amount after discount
            $amount_after_discount = $line_total - $discount;
            $polines[$key]['amount'] = $amount_after_discount;

            // Taxable amount (same as amount after discount, before trade discount)
            $polines[$key]['taxable_amount'] = $amount_after_discount;

            // Accumulate subtotal
            $subtotal = $amount_after_discount + $subtotal;
            $sub_total = $sub_total + $amount_after_discount;
            $tot_tax_amt = $X + $Y;
            $qtytotal += $value->qty;
            $freeqtytotal += $value->free_qty;
        ...
        $this->data['tot_amt_aftr_tax'] = $subtotal + $tot_tax_amt;
        ...
        $this->data['freeqtytotal'] = $freeqtytotal;
        $line_total = $subtotal + $tot_tax_amt;
        $this->data['line_total'] = $line_total;

        $gstdata = \DB::select("select * from s_invoice_lines_t where invoice_hdr_id='" . $id . "' and tax_group_id!='0' group by hsn_code");
        $this->trd = $customer[0]->trade_discount_pre;

        $this->cash_disc = isset($customer[0]->schemes_type_value) ? $customer[0]->schemes_type_value : 0;
        ...
        $grand_total = $sub_total + $gsttotal;
        $this->data['grand_total'] = $grand_total;
        // Purpose to GST calculation End
        ...
        if (isset($_GET['copy'])) {
            ...
            if ($this->data['copy'] == "Original Copy" && $emp_id != 1) {
                $ch = \DB::select("SELECT invoice_hdr_id FROM `s_invoice_hdr_t`  where print_status=0 and invoice_status='APPROVED' and invoice_hdr_id=$id");
                if (count($ch) > 0) {
                    $this->data['copy'] = "Original Copy";
                } else {
                    $this->data['copy'] = "Duplicate Copy";
                }
            }
        } else {
            $this->data['copy'] = '';
        ...
        if (isset($_GET['mail'])) {
            ...
            \Mail::send('salesinvoice.printform', $this->data, function ($message) use ($msg) {
                if (!empty($_GET['cc'])) {
    ...
    public function invoicesoqty($index = null, $pid = null, $sid = null)
    {
        ...
        $html1 .= '<div id="preview-area" class="chandru"><table class="table table-sm table-bordered batch_table"><thead><tr style="background-color:#05234e;color:#fff;"><th>Line No</th><th>Sales Order No</th><th>Order Qty</th><th>Dispatched Qty</th><th>Invoiced Qty</th><th>Invoice Qty</th></tr></thead><tbody>';
        ...
        $html1 .= '</tbody></table>
            <button class="qtyok btn btn-primary" >Add Invoice Qty </button>
            </div>';
        return $html1;
    }
    ...
    public function getdispatchupdate($id = null)
    {
        ...
        $this->data['row']->created_by = $this->jCombo('tb_users', 'id', 'username', \Session::get('id'));
        $this->data['tax_group_id_pop'] = $this->jCombotax('m_tax_group_t', 'tax_group_id', 'tax_group_name', '');
        $tablelines = \DB::table('s_dispatched_qty_t')
            ->leftjoin('s_dispatch_lines_t', 's_dispatch_lines_t.so_dispatch_line_id', '=', 's_dispatched_qty_t.so_dispatch_line_id')
            ->select('s_dispatched_qty_t.*', 's_dispatch_lines_t.*', DB::raw("SUM(s_dispatched_qty_t.issue_qoh) as issue_qoh"))
            ->whereIn('s_dispatch_lines_t.so_dispatch_hdr_id', explode(",", $id))
            ->groupBy('s_dispatched_qty_t.batch_no', 's_dispatch_lines_t.product_id')
            ->orderby('s_dispatched_qty_t.so_dispatch_line_id', 'asc')->get();
        //dd($tablelines);
        $freetablelines = \DB::table('s_dispatched_qty_t')
            ->leftjoin('s_dispatch_lines_t', 's_dispatch_lines_t.so_dispatch_line_id', '=', 's_dispatched_qty_t.so_dispatch_line_id')
            ->select('s_dispatched_qty_t.*', 's_dispatch_lines_t.*', DB::raw("SUM(s_dispatched_qty_t.issue_qoh) as issue_qoh"))
            ->whereIn('s_dispatch_lines_t.so_dispatch_hdr_id', explode(",", $id))
            ->where('s_dispatched_qty_t.free_val', '=', 'free')
            ->groupBy('s_dispatch_lines_t.product_id')
            ->orderby('s_dispatched_qty_t.so_dispatch_line_id', 'asc')->get()->toArray();
        //   dd($freetablelines);
        $freetablelines_product = array_column($freetablelines, 'product_id');
        $freetablelines_free_qty = array_column($freetablelines, 'free_qty');
        ...
        foreach ($tablelines as $key => $value) {
            ...
            if (!empty($salesorder->ship_to_customer_id)) {
                $manufacturerCondition = ' and manufacturer_source_value_id=' . $salesorder->ship_to_customer_id;
            }
            $this->data['subgrid']['rowData'][$key]->part_no = $this->jcustomselecttool('m_manufacturer_partno_t', 'manufacturer_partno_id', 'part_no', $value->part_no, 'and product_id=' . $value->product_id . $manufacturerCondition);
            ...
            $pro_ind = array_search($value->product_id, $freetablelines_product);
            // dd($freetablelines_free_qty[$pro_ind]);
            ...
            if (array_search($value->product_id, $freetablelines_product) !== false) {
                $this->data['subgrid']['rowData'][$key]->free_qty = str_replace(",", "", (number_format($freetablelines_free_qty[$pro_ind], $decimal)));

                unset($freetablelines_product[$pro_ind]);
            } else {
                $this->data['subgrid']['rowData'][$key]->free_qty = 0;
            }
            ...
            if ($value->free_val != 'Free')
                $this->data['subgrid']['rowData'][$key]->qty = str_replace(",", "", (number_format($value->issue_qoh, $decimal))) - $this->data['subgrid']['rowData'][$key]->free_qty;
            else
                $this->data['subgrid']['rowData'][$key]->qty = 0;
            // dd($value);
            $this->data['subgrid']['rowData'][$key]->comments = $value->comments;
            ...
            if ($table[0]->ship_to_customer_id == "" || $table[0]->ship_to_customer_id == "0") {
                $this->data['product'] = $this->jcustomproductselect('m_products_t', 'product_id', 'product_code|concatenated_product', '', 'soquote');
            } else {
                $this->data['product'] = $custaddress1->getcustomerpriceproduct($table[0]->ship_to_customer_id, '');
            }
            if (($table[0]->ship_to_customer_id == "" || $table[0]->ship_to_customer_id == "0") && $value->product_id == "") {
                $this->data['subgrid']['rowData'][$key]->product_id = $this->jcustomproductselect('m_products_t', 'product_id', 'product_code|concatenated_product', '', 'soquote');
            } else if (($table[0]->ship_to_customer_id != "" || $table[0]->ship_to_customer_id != "0") && $value->product_id == "") {
                $this->data['subgrid']['rowData'][$key]->product_id = $custaddress1->getcustomerpriceproduct($table[0]->ship_to_customer_id, $value->product_id);
            } else {
                $this->data['subgrid']['rowData'][$key]->product_id = $this->jcustomselect('m_products_t', 'product_id', 'product_code|concatenated_product', $value->product_id, 'and product_id=' . $value->product_id);
            }
    ...
    function getBill_to_address($bill_to_id = null)
    {
        ...
        $bill_to_address = \DB::select("select * from m_customer_sites_t where customer_id=" . $bill_to_id . " and primary_address = 'YES'");
    ...
    function getGst($gst, $hsn, $po_id)
    {
        ...
        $sql = \DB::SELECT("select * from s_invoice_lines_t where invoice_hdr_id='" . $po_id . "' and tax_group_id='" . $gst . "' and hsn_code='" . $hsn . "'");
    ...
    public function lrupdate(Request $request)
    {
        $updateData = [];

        if ($request->filled('lr_no')) {
            $updateData['lr_no'] = $request->lr_no;
        }

        if ($request->filled('lr_date')) {
            $updateData['lr_date'] = $request->lr_date;
        }

        if ($request->has('lr_status')) {
            $updateData['lr_status'] = $request->lr_status;
        }
        ...
        if ($request->filled('cheque_date')) {
            $updateData['cheque_date'] = $request->cheque_date;
        }

        if ($request->filled('cheque_number')) {
            $updateData['cheque_no'] = $request->cheque_number;
        }

        if ($request->filled('cheque_amount')) {
            $updateData['cheque_amount'] = $request->cheque_amount;
        }

        if ($request->filled('cheque_received_date')) {
            $updateData['cheque_received_date'] = $request->cheque_received_date;
        }

        if ($request->filled('irn_no')) {
            $updateData['irn_no'] = $request->irn_no;
        }

        if ($request->filled('irn_date')) {
            $updateData['irn_date'] = $request->irn_date;
        }
        ...
        $check = \DB::table('s_invoice_hdr_t')
            ->where('invoice_hdr_id', $request->id)
            ->update($updateData);
    ...
    public function exportinvratevalidation($invdate, $invcurrency)
    {

        $sql = \DB::table('f_account_exchangerates_t')->whereDate('from_date', '<=', $invdate)->whereDate('to_date', '>=', $invdate)->where('from_currency_id', $invcurrency)->where('to_currency_id', 37)->where('active', 'Yes')->select('*')->get();
    ...
    public function lredit()
    {
        ...
        $sql = \DB::select("select lr_no,lr_date,lr_status,delivery_date,cheque_no,cheque_date,cheque_amount,cheque_received_date,irn_no,irn_date from s_invoice_hdr_t where invoice_hdr_id='" . $_GET['id'] . "'");
        if (isset($sql)) {
            if ($sql[0]->lr_no != '' || $sql[0]->lr_date != '') {
                ...
                $data['cheque_number'] = $sql[0]->cheque_no;
                $data['cheque_date'] = $sql[0]->cheque_date;
                $data['cheque_amount'] = $sql[0]->cheque_amount;
                $data['cheque_received_date'] = $sql[0]->cheque_received_date;
                $data['irn_no'] = $sql[0]->irn_no;
                $data['irn_date'] = $sql[0]->irn_date;
                $data['update'] = 'update';
    ...
    public function ewayedit()
    {

        $sql = \DB::select("select eway_billno,eway_date from s_invoice_hdr_t where invoice_hdr_id='" . $_GET['id'] . "'");
        if (isset($sql)) {
            if ($sql[0]->eway_billno != '' || $sql[0]->eway_date != '') {
                $data['eway_billno'] = $sql[0]->eway_billno;
                $data['eway_date'] = $sql[0]->eway_date;
                $data['update'] = 'update';
            } else if ($sql[0]->eway_billno == '' && $sql[0]->eway_date == '') {
                $data['eway_billno'] = $sql[0]->eway_billno;
                $data['eway_date'] = $sql[0]->eway_date;
                $data['update'] = 'create';
            }
    ...
    function salesloadtds($id = null, $tds = null)
    {
        ...
        if ($customer->isNotEmpty()) {
            ...
            $tds['tcs_percentage'] = $customer[0]->tcs_percentage;
            $tds['tcs_account_id'] = $customer[0]->tcs_account_id;
            $tds['tcs_applicable'] = $customer[0]->tcs_applicable;
        } else {
            $tds = 0;
        }
        return $tds;
...
where ar_discount_hdr_id = '$id' ";
    ...
    // upload invoice
    public function invoicedocupload(Request $request)
    {
        ...
        if ($request->hasFile('doc_attachment')) {
            ...
            $destinationPath = public_path('uploads/billofloading/' . $id);
            ...
            $dataToUpdate['doc_attachment'] = $filename;


            \DB::table('s_invoice_hdr_t')
                ->where('invoice_hdr_id', $id)
                ->update($dataToUpdate);
