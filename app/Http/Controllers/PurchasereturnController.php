 ...
 /*  purpose for Display Purchase Return Data in JQgrid function */
    public function purchasereturnData(){
      ...
      if($_GET['status']!=''){
          ...
          $wh.=$grid_data=$this->grid_statuscheck('v1','return_date','p_return_status',$op,$val_status);
      } else {
          $wh.=$grid_data=$this->grid_check('v1','return_date');
      }
        ...
        $groupname=\Session::get('groupname');
                ...
                $SQL = "select * from (SELECT
                        ...
                        p_return_header_t.po_tax_total,
                        p_return_header_t.po_grand_total,
...
//            dd($qcspeclns);
        ...
        //dd($dis_minus);        
	$taxamount = (($value->reject_qty * $unitprice) * $display )/ 100;
	//$linetotal = ($taxamount + ($value->reject_qty * $unitprice));
	$linetotal = ($taxamount + $dis_minus);
    //dd($linetotal);
	...
	$this->data['linedata'][$key]->tax_group_id=$this->jCombotax('m_tax_group_t','tax_group_id','tax_group_name',$tax_group_id);
        ...
        $this->data['linedata'][$key]->taxgroup_id =  $this->jCombotax('m_tax_group_t','tax_group_id','tax_group_name',$this->taxgroup($value->product_id));
          ...
          $this->data['row'][0]->po_grand_total=$po_grand_total;
            $this->data['row'][0]->paid_amount="";
          $this->data['row'][0]->balance_amount=$po_grand_total;
}
      ...
      return view('purchasereturn.form',$this->data);
       ...
       if(isset($id))
		...
		$this->data['row'][0]->p_return_status=$return_table[0]->p_return_status;
                $this->data['row'][0]->invoice_number=$return_table[0]->po_invoice_id;
...
/*End Purpose For Edit Function*/
     ...
     public function purchasereturnsave(Request $request)
        {
                ...
                try
                {
                    ...
                    unset($data['pagemode']);
                     ...
                     $po_detail = DB::table('p_po_hdr_t')->where('po_hdr_id',$po_number)->get();
                     $po_date = $po_detail[0]->po_date;
                     $data['po_date'] = $po_date;
                     $data = collect($data)->except(['_token'])->toArray();
                     $data = collect($data)->except(['bill_number'])->toArray();
                     //dd($data);
 ...
 $supacc = DB::table('m_supplier_t')->where('supplier_id', $_POST['supplier_id'])->first();
 if (!$supacc) {
    throw new \Exception("Supplier account not found");
}
 $reverse_charge = DB::table('p_po_invoice_hdr_t')
    ->where('bill_number', $_POST['invoice_number'])
    ->first();
$total=0;
$tkey=1;
          foreach($_POST['bulk_product_id'] as $key=>$value){
			  
			   $filtered = $collection->firstWhere('product_id', $value);

                if (!$filtered) {
                    throw new \Exception("Product not found for product_id: ".$value);
                }

                $accountId = $filtered->control_account_id;
            ...
            $journal_lines_data[$tkey]['account_id']=$accountId;
            $rate=$unit_price-(($unit_price*$discount_percentage)/100);
            $rate=    $rate*$qty ; 
            $tax_details=array();
            if($filtered->tax_credit!="Yes")
            {
		...
		$total=$total+$rate;
			...
			$tkey++;
            if (($reverse_charge->reverse_charge ?? '') == '') {
            foreach($tax_details as $taxval)
            {
		       ...
		       $tkey=0;
                ...
                $journal_lines_data[$tkey]['account_id']=$supacc->account_structure_id;
 ...
 $journal_lines_data=array();
   ...
   if (!empty($reverse_charge->reverse_charge ?? '')) {
...
/*End Purpose For View Function*/
    ...
    /* Purpose For Purchase Return Approval Function*/
    public function purchasereturnapproval($id=null,$aprv=null)
       ...
       if(isset($id))
		...
		$this->data['row'][0]->p_return_status=$return_table[0]->p_return_status;
                $this->data['row'][0]->invoice_number=$return_table[0]->po_invoice_id;
		//dd($return_table[0]->po_invoice_id);
                $this->data['supplier_id']= $this->jCombo('m_supplier_t','supplier_id','supplier_name','');
                $this->data['subcontract_supplier_id']= $this->jCombo('m_subcontract_supplier_t','subcontract_supplier_id','subcontract_name','');
                if($return_table[0]->supplier_id !=""){
                    $this->data['supplier_id']= $this->jCombo('m_supplier_t','supplier_id','supplier_name',$return_table[0]->supplier_id);
...
/*Karthigaa Purpose for Print*/
    public function getprint($id=null)
	{
	 ...
	 if(isset($id))
    	{
    	    $gin_table=\DB::table('p_return_header_t')->select('p_po_hdr_t.po_number','p_po_hdr_t.suppliersite_id','m_subcontract_supplier_t.subcontract_supplier_id','p_po_hdr_t.po_hdr_id','m_supplier_t.supplier_id','p_po_hdr_t.po_number','p_po_hdr_t.po_date','p_return_header_t.dc_number','p_grn_hdr_t.grn_number','p_grn_hdr_t.grn_id','p_grn_hdr_t.dc_date','p_return_header_t.return_header_id','p_return_header_t.return_date','p_return_header_t.po_tax_total','p_return_header_t.po_grand_total','p_return_header_t.return_invoice_number','p_po_invoice_hdr_t.po_invoice_id','p_po_invoice_hdr_t.dc_number','p_qc_header_t.qc_header_id','p_po_invoice_hdr_t.bill_number','p_po_invoice_hdr_t.invoice_date','p_qc_header_t.qc_number','p_qc_header_t.qc_date')
			->leftJoin('m_supplier_t', 'm_supplier_t.supplier_id', '=', 'p_return_header_t.supplier_id')
            ->leftJoin('m_subcontract_supplier_t', 'm_subcontract_supplier_t.subcontract_supplier_id', '=', 'p_return_header_t.subcontract_supplier_id')
			->leftJoin('p_po_hdr_t','p_po_hdr_t.po_hdr_id','=','p_return_header_t.po_number')
			->leftJoin('p_grn_hdr_t','p_grn_hdr_t.grn_id','=','p_return_header_t.grn_number')
		...
		->leftJoin('p_po_invoice_hdr_t','p_po_invoice_hdr_t.po_invoice_id','=','p_return_header_t.invoice_number')
			...
			{ 
                ...
                $this->data['po_number']=$gin_table[0]->po_number;
                $this->data['po_date']=date('d-m-Y',strtotime($gin_table[0]->po_date));
                $this->data['grn_number']=$gin_table[0]->grn_number;
                $this->data['dc_date']=date('d-m-Y',strtotime($gin_table[0]->dc_date));
                $this->data['qc_number']=$gin_table[0]->qc_number;
                $this->data['qc_date']=date('d-m-Y',strtotime($gin_table[0]->qc_date));
                $this->data['bill_number']=$gin_table[0]->bill_number;
                $this->data['invoice_date']=date('d-m-Y',strtotime($gin_table[0]->invoice_date));
		...
		$this->data['return_header_id']='';
		 	...
		 	if($tablelines->isNotEmpty())
			{
				...
				foreach($this->data['linedata'] as $key => $value)
				{
    			    ...
    			    $tot=$value->reject_qty * $value->unit_price;
				}
			$this->data['tot']=$tot;
			$this->data['tax']=$tax;
			}
	...
	if(!empty($gin_table[0]->supplier_id))
	{
	  ...
	  /*********** supplier Address *************/
		...
		if(!empty($gin_table[0]->supplier_id))
		{
    		$supplier_address=$this->getSupplieraddress($gin_table[0]->suppliersite_id);
                // dd($supplier_address);
...
// Purpose to get Locaion Address for Print*/
...
/*Karthigaa Purpose to get Supplier Location Address for Print*/
		...
		$supplier_address=array();
    	        $supplier_address=\DB::select("select * from m_supplier_sites_t where supplier_site_id='$sub_id'");
		if(!empty($supplier_address))
		{
...
/*Karthigaa Purpose to get Supplier for Print*/
	...
	public function returncreditupdate($id=null) {
      $expense_id = $id;
      ...
      $credit_date = date('Y-m-d',strtotime($_POST['credit_date']));
       if($expense_id!=""){
           ...
           \DB::table('p_return_header_t')->whereIn('return_header_id',$exp_id)->update(['credit_taken' => $credit_taken,'credit_date'=>$credit_date]);
