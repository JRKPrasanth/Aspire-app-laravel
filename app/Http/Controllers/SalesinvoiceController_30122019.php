...
//dd($_POST['invoice_currency']);
       ...
       $custable = \DB::table('m_customers_t')->join('m_customer_types_t','m_customer_types_t.customer_type_id','=','m_customers_t.customer_type_id')->where('customer_id','=',$cid)->select('m_customers_t.account_structure_id','m_customer_types_t.account_id')->get();
...
$tkey=0;
            ...
            $journal_lines_data[$tkey]['reference_id']=$cid;
    ...
    //     echo "<pre>";
    //     print_r(count($sales_account_id));
    //     exit();
    // dd($_POST);
        
//     dd($_POST);
  //other Charges,tax,Freight Amount
...
public function invoicesoqty($index=null,$pid=null,$sid=null){
    ...
    if($check_qty == '0'){
        ...
        //salesorder from dispacth
                $dispact_data =\DB::select('SELECT * from s_dispatch_lines_t where so_dispatch_hdr_id in('.$dispatch.') and product_id='.$pid); 
$dispdata=0;
            foreach($dispact_data as $dk=>$dv){
   $dispdata.=$dv->soorder_id.",";
}
       ...
       $this->data['pay_term']=$this->idname('payment_term_name','m_payment_terms_t','payment_term_id',$customer[0]->payment_term_id);
        ...
        $vlinesdata = DB::table('s_invoice_lines_t')
            ...
            ->select('s_invoice_lines_t.line_no',
                   ...
                   'm_tax_group_t.tax_group_name')->where('invoice_hdr_id',$id)->groupBy('s_invoice_lines_t.product_id')->get();  
