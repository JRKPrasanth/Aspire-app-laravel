...
$('table.preview').stickyTable({style : 'stripped'});
		...
		this.each(function() {
			...
			$(this).click(function(){
				...
				$(rel).find('select').each(function (i, obj) {
					$(obj).select2("destroy");
				});
				...
				// $(clone).find('.select2-container').unbind();
			//	$(clone).find('select').select2("destroy");
              ...
              maxDate: null,})
				...
				// $(clone).find('select.select2').each(function (i, obj) {
				// 	console.log(obj);
				...
				$(parent).find('select').each(function (i, obj) {
					$(obj).select2();
				});

				$(parent).find('preview').each(function (i, obj) {
					//$(obj).select2({width:'100%'});
					$(obj).stickyTable({style : 'stripped'});
				});
